...
본 지침서는 HnVue GUI Console SW의 제품개발 전 단계에 걸쳐 IEC 62304 Class B 소프트웨어 개발 표준을 준수하는 **체계적이고 반복 가능한(repeatable)** 개발 프로세스를 정의한다.
...
- HnVue과 연결되는 X-Ray 발생장치(Generator)의 펌웨어
...
HnVue SW 개발은 **V-Model** 기반의 생명주기 모델을 채택한다. V-Model은 좌측 하향 경로(개발)와 우측 상향 경로(테스트)로 구성되며, 각 개발 단계는 대응하는 검증/밸리데이션 단계와 직접 연결된다.
...
### 4.2 HnVue의 Class B 근거

HnVue GUI Console SW는 **IEC 62304 Class B**로 분류된다. 분류 근거는 다음과 같다:
...
flowchart TD
    ...
    G[HnVue SW 결함 시나리오] --> H{촬영 파라미터 오표시}
    H --> I[방사선 과피폭 — 중등도]
    H --> J[잘못된 환자 데이터 표시]
    J --> K[오진 위험 — 중등도]
    G --> L{UI 응답 오류}
...
| IEC 62304 요구사항 | Class A | Class B | Class C | HnVue 적용 |
|-------------------|---------|---------|---------|-----------------|
| SW 개발 계획 (§5.1) | ✓ | ✓ | ✓ | ✓ |
| SW 요구사항 (§5.2) | ✓ | ✓ | ✓ | ✓ |
| SW 아키텍처 설계 (§5.3) | — | ✓ | ✓ | ✓ |
...
HnVue 프로젝트는 3단계 요구사항 계층 구조를 사용한다:
...
FDA 21 CFR 820.30은 의료기기 소프트웨어 설계의 전 과정에 대한 통제 요구사항을 정의한다. HnVue은 다음 Design Controls 프로세스를 준수한다:
...
FDA Section 524B 및 FDA Cybersecurity Final Guidance(2023)에 따라 HnVue은 **Secure SDLC**를 전 개발 단계에 통합한다:
...
STRIDE 방법론을 적용하여 HnVue의 보안 위협을 체계적으로 분석한다:

| 위협 유형 | 설명 | HnVue 적용 예 | 대응 통제 |
|----------|------|-------------------|----------|
...
| 패스워드 저장 | bcrypt (cost factor=12) 해시 (평문 저장 절대 금지) |
...
*본 문서는 HnVue GUI Console SW 개발팀의 공식 지침서입니다.*  
*문서에 관한 문의사항은 SW 개발팀 또는 QA팀에 연락하시기 바랍니다.*

---

**문서 끝 (End of Document)**  
DOC-RADI-GL-001 v1.0 | HnVue SW 제품개발 지침서
