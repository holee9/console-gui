...
| **개발자 2** | 1명 | Team A 보조 | 인프라 보조 (Common, Data, Security, SystemAdmin, Update) |
| **디자이너** | 1명 | Team Design | PPT/Figma→XAML 코드화 (기능구현 없음, 순수 디자인) |
| **QA** | 1명 | QA | CI/CD, 커버리지 분석, 릴리스 준비도 보고, 코드 리뷰 |
| **RA** | 1명 | RA | IEC 62304 문서, SBOM, RTM, FDA 510(k) 제출 준비 |

**겸임 규칙:**
- SW 개발팀장: Coordinator(통합) + Team A(인프라) 겸임. 코드 리뷰 최종 승인자
- 개발자 2: Team A 보조로 인프라 개발에 집중
- 디자이너: Team Design 전담, XAML 코드화만 수행 (C# 기능구현 불가)
- Design Team은 코더가 없으므로 C# 기능구현은 SW팀장(Coordinator)과 개발자들이 분담
- AI 에이전트(MoAI)가 각 워크트리 팀의 실행을 보조하여 인력 효율 극대화
