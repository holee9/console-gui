...
| **테스트** | **812개** 전체 통과 ✅ (단위 794 + 통합 18, +69 신규) |
| **품질 점수** | 0.91/1.0 ✅ |
...
| **Gitea 이슈** | 40개 등록, 39개 해결 (98%) — 2026-04-06 기준 |
| **1차 릴리즈 준비도** | ~82% — 이미지 처리 파이프라인·UI 화면·하드웨어 연동 보강 |
...
Total: 812 tests completed in ~90 seconds
```
...
| `src/HnVue.Workflow/README.md` | X-ray 촬영 워크플로우 엔진 |
...
총 **70개** 활성 문서가 6개 카테고리로 관리됩니다. 자동 동기화 스크립트 (`scripts/sync_docs.py`)로 버전 일관성을 유지합니다.

### 기획 문서 (`docs/planning/`)

| Doc ID | 문서명 | 버전 |
|--------|--------|:----:|
| **DOC-001** | MRD (시장 요구사항) | v3.0 |
| DOC-001a | MR Detailed Spec — Tier 1 (인허가 필수 13개) | v1.0 |
| DOC-001b | MR Detailed Spec — Tier 2/3/4 (시장 진입 + 차별화) | v1.0 |
| **DOC-002** | PRD (제품 요구사항) | v2.0 |
...
| CVR-002 | Final Cross Verification | v1.0 |
...
**문서 최종 업데이트:** 2026-04-06  
**프로젝트 상태:** Phase 1 코드 기반 완성 (812개 테스트, 0.91/1.0) — 1차 릴리즈 준비도 ~82%  
**현재 단계:** Phase 1.5 — Gap 분석 & 시험 보고서 현실화  
**다음 단계:** Phase 2 — HnVue.Imaging 실구현 + WPF UI 화면 완성  
