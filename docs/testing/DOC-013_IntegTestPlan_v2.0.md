...
## 11. Generator 통합 에러 처리 테스트

| TC ID | 테스트명 | 시나리오 | 예상 결과 | MR 추적 |
|-------|---------|---------|----------|--------|
| IT-GEN-001 | Generator 통신 끊김 → 자동 재연결 | 시리얼 케이블 분리 후 재연결 | 3회 재시도 후 복구, SS-IDLE | MR-031 |
| IT-GEN-002 | Exposure 중 E09 → 촬영 중단 + 이미지 폐기 | E09 시뮬레이션 | Exposure 즉시 중단, 이미지 미저장 | MR-031 |
| IT-GEN-003 | E36 Tube 과열 → 냉각 후 촬영 재개 | E36 시뮬레이션 | 냉각 카운트다운 표시, 해제 후 SS-IDLE | MR-031 |

## 12. 안전 상태 통합 전환 테스트

| TC ID | 테스트명 | 시나리오 | 예상 결과 |
|-------|---------|---------|----------|
| IT-SS-001 | PACS + Generator 동시 장애 | 두 인터페이스 모두 다운 | SS-BLOCKED, 촬영 차단, 로컬 저장 |
| IT-SS-002 | 장애 복구 순서 | PACS 복구 → Generator 복구 | SS-DEGRADED → SS-IDLE 순차 복귀 |
| IT-SS-003 | FPD 교체 중 워크플로우 | 디텍터 1 → 2 전환 실패 | 디텍터 1 롤백, SS-DEGRADED |
