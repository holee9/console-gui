...
| **버전 (Version)** | v1.0 |
| **제품명 (Product Name)** | RadiConsole™ GUI Console SW |
| **작성일 (Date)** | 2026-03-16 |
...
본 V&V 마스터 플랜 (Verification & Validation Master Plan)은 의료용 진단 X-Ray 촬영장치의 GUI Console Software인 **RadiConsole™ GUI Console SW**에 대한 검증 (Verification) 및 밸리데이션 (Validation) 활동의 전략, 방법론, 일정, 책임 및 합격 기준을 정의한다.
...
**적용 대상**: RadiConsole™ GUI Console SW **Phase 1** (M1~M12 핵심 기능)
...
RadiConsole™ GUI Console SW는 **IEC 62304 Class B** (소프트웨어 고장이 심각한 부상 (SERIOUS INJURY)을 유발할 수 있으나 사망을 유발할 가능성이 낮음)로 분류되어 있으며, 이에 따른 검증 요건이 적용된다.
...
RadiConsole™ V&V는 IEC 62304 및 FDA 권장 방법론에 따라 **V-Model** 기반 전략을 채택한다. 개발 계층 (좌측)과 검증/밸리데이션 계층 (우측)이 1:1 대응 관계를 형성한다.
...
RadiConsole™ V&V는 다음 4가지 검증 방법 (TIAD)을 조합하여 적용한다:
...
```mermaid
C4Context
    title RadiConsole™ 통합 테스트 컨텍스트

    System_Boundary(gui, "RadiConsole™ GUI SW") {
...
시스템 테스트 (System Test)는 완전히 통합된 RadiConsole™ SW가 FRS 및 PRD (PR-xxx)에 정의된 모든 기능 및 비기능 요구사항을 충족함을 검증한다. IEC 62304 §5.8 요건에 따라 수행되며, 실제 사용 환경과 유사한 스테이징 환경에서 실시한다.
...
설계 밸리데이션 (Design Validation)은 FDA 21 CFR 820.30(g) 요건에 따라 최종 출시 빌드의 RadiConsole™ SW가 의도된 사용자 및 사용 환경에서 사용자 요구사항 (MRD)을 충족함을 확인한다. 실제 사용자 대표 집단 (Representative Users)이 참여하는 테스트를 포함한다.
...
FDA Section 524B (Cybersecurity Guidance) 및 FDA 21 CFR 820.30에 따라 RadiConsole™ SW의 사이버보안 위험을 검증한다. AAMI TIR57 및 IEC 81001-5-1 프레임워크를 참조한다.
...
```mermaid
gantt
    title RadiConsole™ V&V 마스터 일정 (Phase 1: M1-M12)
    dateFormat  YYYY-MM
    axisFormat  %Y-%m
...
*본 문서는 RadiConsole™ GUI Console SW Phase 1에 대한 공식 V&V 마스터 플랜이다. 이후 변경 사항은 SCP-XRAY-001 (소프트웨어 구성 관리 계획)에 따라 공식 변경 통제 (Change Control) 절차를 통해 관리된다.*
...
---
*VVP-XRAY-GUI-001 v1.0 | © 2026 RadiConsole™ Project | IEC 62304 Class B | FDA 510(k) / CE MDR / KFDA*
