...
```
WorkflowEngine(IDoseService, IGeneratorInterface, ISecurityContext, IAuditService? = null)
```
...
2. `Exposing` 또는 `ReadyToExpose` 상태인 경우 `IGeneratorInterface.AbortAsync()` 호출
3. 제너레이터 중단 실패 시 `SafeState.Emergency`로 에스컬레이션
4. 워크플로우 상태를 `Complete`로 전이
...
| 파일 | 테스트 수 | 내용 |
|---|---|---|
| `WorkflowEngineTests.cs` | 27 | 선량 인터락 4단계, RBAC, 상태 전이, Abort 에스컬레이션, 감사 로그 |
...
| `GeneratorSerialPortAdapterTests.cs` | 17 | ISerialPortAdapter 테스트용 FakeAdapter, DI 주입 시나리오 |

**전체 테스트: 86 → 106개** (+20개, GeneratorSerialPortAdapterTests 신규 17개)
