...
| HnVue.Data | ✅ 완료 | EF Core DbContext, 6개 DbSet (Patient, Study, Image, DoseRecord, User, AuditLog) |
| HnVue.Security | ✅ 완료 | JWT, RBAC, PasswordHasher, SecurityContext — 완전 구현 |
...
| HnVue.Dicom | ✅ 완료 | DicomFileIO (fo-dicom), DicomStoreScu, DicomFindScu — 실제 구현 |
| HnVue.App | ✅ 완료 | DI 컴포지션 루트 (13개 모듈 등록) — Wave B에서 스텁 교체 예정 |

### 2.2 미완성 모듈

| 모듈 | 상태 | 핵심 문제점 |
|------|------|------------|
| HnVue.Imaging | ⚠️ 스텁 | `Math.Sqrt(pixelCount)`로 크기 추정, DICOM 헤더 파싱 없음, Pan/Zoom 미구현 |
...
| IWorklistRepository | App.xaml.cs 내 NullWorklistRepository | 없음 — DICOM MWL 기반 | DB 불필요, DicomFindScu 연동 |
...
Layer 5: HnVue.UI [LoginView만 완성 → 4개 View 부재]
         │
Layer 6: HnVue.App [DI완료, 스텁 교체 대기]
```
...
|---|------|------|------|---------|----------|------|
| T-01 | HnVue.Imaging DICOM 헤더 파싱 | **병렬** | A | WT-1 | 없음 | IImageProcessor 인터페이스 안정, 단일 파일 소유 |
...
| T-08 | DoseRepository EF 구현 | **병렬** | A | WT-3 | 없음 | DoseRecords DbSet 이미 존재, DbContext 변경 불필요 |
| T-09 | WorklistRepository DICOM MWL 구현 | **병렬** | A | WT-3 | 없음 | DicomFindScu 연동, DB 불필요 |
| T-10 | IncidentRepository EF 구현 | **병렬** | A | WT-3 | 없음 | 단, IncidentEntity + DbSet 추가 필요 (WT-3 소유) |
| T-11 | SystemSettingsRepository 구현 | **병렬** | A | WT-3 | 없음 | 파일 기반 JSON 또는 SQLite, WT-3 소유 |
| T-12 | UpdateRepository 파일시스템 구현 | **병렬** | A | WT-3 | 없음 | 패키지 파일 스캔, DB 불필요 |
| T-13 | CDBurning.StudyRepository EF 구현 | **병렬** | A | WT-3 | 없음 | 기존 DbSet 활용 |
...
| T-17 | App.xaml.cs Null 스텁 교체 | **단독** | B | 없음 | T-08~T-13 완료 | DI 컴포지션 루트, 단일 파일 |
| T-18 | GeneratorSimulator → SerialPort 교체 | **단독** | B | 없음 | T-14 완료 | App.xaml.cs 수정 |
| T-19 | 단위 테스트 (Imaging, UI ViewModels) | **병렬** | B | 선택적 WT | T-01~T-07 | 인터페이스 기반 모킹 가능 |
| T-20 | 통합 테스트 (End-to-End 워크플로) | **단독** | C | 없음 | T-15~T-18 완료 | 전체 파이프라인 검증 |
| T-21 | IEC 62304 소프트웨어 검증 테스트 | **단독** | C | 없음 | T-20 완료 | 규제 필수, 단독 실행 |
...
| `HnVue.UI/ViewModels/MainViewModel.cs` | **없음** | 없음 | Wave B 단독 수정 |
...
- [ ] WT-1: `ImageProcessor` 단위 테스트 통과, 실제 DICOM 파일로 픽셀 데이터 정상 추출
- [ ] WT-2: 4개 View가 DI 컨테이너에서 인스턴스화 가능, 기본 데이터 바인딩 동작
...
- [ ] IEC 62304 §5.7 소프트웨어 통합 테스트 레코드 작성
- [ ] 커버리지 85% 이상 (단위 + 통합 통합)
