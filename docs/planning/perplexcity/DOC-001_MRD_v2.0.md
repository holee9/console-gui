...
## 의료용 진단 X-Ray 촬영장치 GUI Console Software
...
| 버전 | 날짜 | 개정자 | 개정 내용 |
|------|------|--------|---------|
| v1.0 | 2026-03-16 | 제품전략팀 | 최초 작성 |
| v2.0 | 2026-03-16 | 제품전략팀 | 인허가 추적성 체계(RTM) 연동을 위한 ID 체계 정비 및 검증/밸리데이션 방법 추가. 각 MR 항목에 연결 PRD ID, 검증 방법, 밸리데이션 방법, 위험 참조, Design Input 분류 컬럼 추가. Design Input Baseline 섹션 신설. 추적성 흐름도 및 V-Model 다이어그램 추가. 부록 D(MRD-PRD 추적성 매트릭스) 및 부록 E(MR-위험관리 연결 매트릭스) 신설. |
...
본 문서는 의료용 진단 X-Ray 촬영장치에 탑재되는 **GUI Console Software (이하 "콘솔 SW")**의 시장 요구사항을 정의한다. 콘솔 SW는 방사선사(Radiographer)가 X-Ray 촬영을 수행하는 전 과정—환자 등록, 검사 프로토콜 선택, 촬영 파라미터 설정, 영상 획득 및 후처리, PACS/RIS 전송—을 통합적으로 제어하는 핵심 사용자 인터페이스 소프트웨어이다.
...
#### 2.1.1 글로벌 의료 영상 소프트웨어 시장

글로벌 Medical Imaging Software 시장은 2025년 **USD 8.74B**에서 2031년 **USD 13.90B**으로 성장할 전망이며, CAGR **8.02%**를 기록할 것으로 예상된다 (출처: Mordor Intelligence).
...
#### 2.1.3 디지털 방사선 촬영(DR) 장비 시장

Digital Radiography Equipment 시장은 2024년 **USD 3.2B**에서 2033년 **USD 5.5B**으로 CAGR **6.5%**의 성장이 전망된다 (출처: LinkedIn Market Analysis). X-Ray Imaging Software 세그먼트는 2026~2033년 동안 CAGR **약 14%**의 고성장이 예측된다.
...
| 기능 카테고리 | Carestream | Siemens | Canon | Fujifilm | OR Tech | Philips |
|------------|:---------:|:-------:|:-----:|:--------:|:-------:|:-------:|
| **AI 기반 이미지 최적화** | ◎ | ◎ | ○ | △ | ○ | ◎ |
| **Smart Positioning** | ○ | ◎ | △ | △ | △ | ○ |
| **Auto Collimation** | ○ | ◎ | ○ | △ | △ | ○ |
...
| **Cloud 연동** | ○ | ○ | △ | △ | △ | ◎ |
| **사이버보안 내재화** | ◎ | ◎ | ○ | ○ | ○ | ◎ |
| **FDA AI 승인 수** | N/A | 80개 | 35개 | N/A | N/A | 42개 |
...
quadrantChart
    ...
    Target Product: [0.82, 0.78]
```

> **전략적 포지셔닝**: 당사 제품(Target Product)은 Carestream/Canon의 높은 사용성과 Siemens/Philips의 높은 기능성을 결합한 **우상단 포지셔닝**을 목표로 한다.
...
> **본 섹션은 v2.0에서 신규 추가된 섹션으로, FDA 21 CFR 820.30 Design Controls 및 ISO 13485:2016 7.3 Design and Development 요건에 따라 MRD가 Design Input의 최상위 출발점임을 명시한다.**
...
2025년 중반 기준, FDA 승인 AI/ML 기반 의료기기 알고리즘은 **873개 이상**에 달하며, 이 중 방사선과(Radiology) 분야가 **78%** 이상을 차지한다. 주요 벤더별 FDA 승인 AI 알고리즘 보유 현황은 GE(96개), Siemens(80개), Philips(42개), Canon(35개)이다.
...
> **본 부록은 v2.0에서 신규 추가된 섹션이다.** MRD의 각 MR-xxx 항목이 분해되어 PRD의 FR-xxx/NFR-xxx로 연결되는 매핑을 요약한다. PRD 개발 시 본 매핑을 기반으로 역방향 추적이 가능해야 한다.
...
> **본 부록은 v2.0에서 신규 추가된 섹션이다.** ISO 14971 위험 관리 프로세스와의 연결을 위해, 각 MR 항목에 관련된 잠재적 Hazard(위험 요소) 카테고리를 매핑한다. 위험 관리 파일(Risk Management File) 작성 시 본 매트릭스를 기준으로 상세 FMEA/FTA를 수행한다.
...
*본 문서는 MRD-XRAY-GUI-001 v2.0이며, 2026년 3월 16일 기준으로 작성되었습니다. 인허가 추적성 체계(RTM) 연동을 위해 v1.0에서 개정되었으며, 시장 상황, 규제 변화, 기술 트렌드에 따라 주기적인 업데이트가 필요합니다.*

*문의: 제품전략팀 | 최종 검토: 기술개발팀, 마케팅팀, 규제팀*
