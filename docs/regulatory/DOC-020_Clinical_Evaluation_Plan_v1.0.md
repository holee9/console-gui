...
| **제목** | RadiConsole™ GUI Console SW 임상 평가 계획서 |
| **버전** | v1.0 |
| **작성일** | 2026-03-16 |
...
본 임상 평가 계획서 (Clinical Evaluation Plan, CEP)는 EU MDR 2017/745 Annex XIV Part A 및 MEDDEV 2.7/1 Rev.4 지침에 따라 RadiConsole™ GUI Console SW의 임상 평가를 체계적으로 계획하고 수행하기 위한 문서이다.

본 CEP의 목적은 다음과 같다:

1. **안전성 (Safety)** 확인: RadiConsole™ GUI SW가 의도된 용도로 사용될 때 허용 가능한 수준의 위험만 존재함을 임상 데이터를 통해 입증
...
- **대상 제품**: RadiConsole™ GUI Console SW (SW Safety Class: IEC 62304 Class B)
...
### 1.3 RadiConsole™의 임상적 역할 정의

> **중요 분류 (Important Classification)**
>
> RadiConsole™ GUI SW는 **진단 보조 소프트웨어 (Diagnostic Aid Software)**로서 의료영상의 최종 진단적 해석 (Diagnostic Interpretation)을 수행하지 않는다. 본 소프트웨어는 방사선사 (Radiologic Technologist) 및 의학물리사가 X-Ray 촬영 장치를 제어하고, 취득된 영상의 품질을 확인하며, DICOM 표준에 따라 영상을 전송·저장하는 **워크플로우 제어 소프트웨어 (Workflow Control Software)**이다.

```mermaid
flowchart LR
    A["방사선사\n(Radiologic Technologist)"] --> B["RadiConsole™\nGUI SW\n(워크플로우 제어)"]
    B --> C["X-Ray 발생장치\n(X-Ray Generator)"]
    B --> D["디지털 디텍터\n(Digital Detector)"]
    B --> E["PACS / HIS\n(DICOM 전송)"]
    B --> F["영상 표시\n(Image Display)"]
    ...
    note1["RadiConsole™는 진단 보조(Diagnostic Aid)로서\n최종 진단은 영상의학과 의사가 수행"]
```
...
**한국어 (Korean):**
RadiConsole™ GUI Console SW는 의료기관 내에서 방사선사 및 의학물리사가 디지털 X-Ray 촬영 장치(Digital Radiography System)를 제어하고 관리하기 위한 소프트웨어이다. 본 소프트웨어는 환자 정보 관리, 촬영 프로토콜 설정, X-Ray 발생 제어, 디지털 영상 취득·표시·전송, 방사선 선량 모니터링, DICOM 통신 및 시스템 관리 기능을 제공한다.
...
| **I (Intervention)** | 중재 (제품) | 디지털 방사선 촬영 콘솔 소프트웨어 사용 (RadiConsole™ 또는 동등 기기) |
...
EU MDR 2017/745 Annex XIV Part A 및 MEDDEV 2.7/1 Rev.4에 따라, RadiConsole™의 임상 평가는 주로 **동등 기기 (Equivalent Device) 데이터를 활용**하는 경로를 적용한다. 이는 해당 기기 유형에 대한 충분한 기존 임상 데이터가 존재하며, 신규 임상 시험 수행이 비윤리적이거나 불필요한 경우에 적용 가능하다.
...
| 특성 (Characteristic) | RadiConsole™ | Predicate A (Carestream DRX) | Predicate B (Siemens MOBILETT) | 동등 여부 |
|----------------------|-------------|------------------------------|-------------------------------|-----------|
...
| **DICOM 서비스** | MWL, Storage, MPPS, Print | MWL, Storage, MPPS, Print | MWL, Storage, MPPS | ✅ 동등 (RadiConsole™ 추가 기능 포함) |
...
| **운영 환경** | 고정식 + 이동식 DR | 고정식 + 이동식 DR | 이동식 DR | ✅ 동등 (RadiConsole™ 범위 포괄) |
...
RadiConsole™과 두 Predicate Device 모두 디지털 방사선 촬영 장비의 콘솔 소프트웨어로서 방사선사가 X-Ray 촬영을 제어·관리하는 동일한 의도된 용도를 가진다. ✅ **충족**
...
RadiConsole™은 소프트웨어 전용 제품으로서 환자 또는 사용자의 신체와 직접 접촉하지 않는다. 따라서 생물학적 동등성 평가는 **해당 없음 (Not Applicable)**이다.
...
```mermaid
quadrantChart
    title Risk-Benefit 비율 평가 (RadiConsole™ GUI SW)
...
**결론:** RadiConsole™ GUI SW의 모든 잔여 위험은 허용 가능한 수준으로 관리되며, 임상적 이익이 잔여 위험을 명백히 초과한다. EU MDR 2017/745 Article 61(1) 요건에 따른 긍정적 Risk-Benefit 비율을 확인한다.
...
| 요소 | RadiConsole™ | Carestream (K183456*) | 동등 여부 |
|------|-------------|----------------------|-----------|
...
| **RadiConsole™ 관련성** | High / Medium / Low |
| **주요 인용 내용** | |
| **평가자** | |
| **평가일** | |
