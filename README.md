...
| **FDA (Food and Drug Administration) Predicate** | DRTECH EConsole1 ([FDA K231225](https://www.accessdata.fda.gov/cdrh_docs/pdf23/K231225.pdf)) |
| **IEC (International Electrotechnical Commission) 62304 분류** | Class B (Software Safety Classification) |
| **인허가 대상** | MFDS (Ministry of Food and Drug Safety) 2등급, FDA (Food and Drug Administration) 510(k), CE MDR Class IIa |
...
- **촬영 워크플로우**: 환자 선택 → 프로토콜 로드 → 촬영 준비 → 노출 → 영상 획득 → PACS (Picture Archiving and Communication System) 전송
- **DICOM (Digital Imaging and Communications in Medicine) 상호운용성**: C-STORE SCU (영상 전송), C-FIND SCU (Worklist 조회), DICOM 3.0 파일 I/O
- **방사선 선량 관리**: IEC (International Electrotechnical Commission) 60601-1-3 준수 검증 (4단계 인터록: ALLOW/WARN/BLOCK/EMERGENCY)
- **보안 및 인증**: JWT (JSON Web Token) 기반 인증, RBAC (Role-Based Access Control) 4역할 (Radiographer/Radiologist/Admin/Service), 감사 로그 (HMAC (Hash-based Message Authentication Code)-SHA (Secure Hash Algorithm) 256 해시 체인)
- **소프트웨어 업데이트**: 무결성 검증 (SHA (Secure Hash Algorithm)-256), 백업/복원 (Timestamp), 코드 서명 검증
- **인시던트 대응**: 4단계 심각도 분류 (Critical/High/Medium/Low), 긴급 콜백
- **미디어 처리**: CD (Compact Disc)/DVD (Digital Versatile Disc) 미디어에 영상 배포 (IMAPI2 시뮬레이션)
- **UI (User Interface)/UX (User Experience) 설계**: PPT 명세 기반 WPF (Windows Presentation Foundation) 화면 구현 — 로그인(드롭다운), Worklist 필터, Studylist, 환자/Procedure 통합 등록, Sync Study, Settings 재설계
...
| 항목 | 기술 | 설명 |
|------|------|------|
| **UI (User Interface) Framework** | WPF (Windows Presentation Foundation) + .NET 8 LTS | MVVM (Model-View-ViewModel) 패턴, MahApps.Metro 테마 |
| **데이터베이스** | SQLite + EF (Entity Framework) Core 8 | SQLCipher AES (Advanced Encryption Standard)-256 암호화 |
| **DICOM** | fo-dicom 5.2.5 (MIT) | C-STORE, C-FIND MWL, DICOM 파일 I/O |
| **인증/보안** | bcrypt (cost=12), JWT (JSON Web Token) HS256, HMAC (Hash-based Message Authentication Code)-SHA (Secure Hash Algorithm) 256 | 비밀번호 해싱, 토큰 기반 인증, 감사 로그 체인 |
| **테스트** | xUnit + NSubstitute + FluentAssertions | 솔루션 기준 테스트 프로젝트 17개 (단위·아키텍처·UI·UI QA·통합) |
| **로깅** | Serilog | SHA (Secure Hash Algorithm)-256 해시 체인, 365일 보관, 감사 추적 |
| **빌드** | dotnet CLI + Visual Studio MSBuild | 자동화 빌드, SBOM (Software Bill of Materials) (CycloneDX), 코드 서명 |
...
| 표준 | 적용 | 설명 |
|------|:----:|------|
| **IEC (International Electrotechnical Commission) 62304:2015+A1** | Class B | 소프트웨어 수명주기 프로세스 (SDLC) |
| **IEC (International Electrotechnical Commission) 62366-1:2015+A1** | 필수 | 사용성 공학 (Usability Engineering) |
| **ISO (International Organization for Standardization) 14971:2019** | 필수 | 위험 관리 (Risk Management) |
| **IEC (International Electrotechnical Commission) 81001-5-1:2021** | 필수 | 사이버보안 수명주기 (Cybersecurity Lifecycle) |
| **FDA (Food and Drug Administration) 21 CFR 820.30** | 필수 | Design Controls |
| **FDA (Food and Drug Administration) Section 524B** | 필수 | SBOM (Software Bill of Materials) + CVD + Patch/Update 관리 |
...
| **PENCIL_DESIGN_SUMMARY** | `docs/ui_mockups/` | Pencil 디자인 도구 요약 |

> **참고**: HTML mockup 파일(`docs/ui_mockups/*.html`)은 더 이상 사용되지 않습니다. UISPEC 문서와 PPT 디자인 소스를 직접 참조하여 XAML을 구현하세요.
...
> **현재 1.15M차 / 12M, 소진 2.3 MM / 24~36 MM** | 상세: [PROGRESS-001](docs/management/PROGRESS-001_Status_Report_v1.0.md)
...
> 상세 진도: [PROGRESS-001](docs/management/PROGRESS-001_Status_Report_v1.0.md) | 교차검증: [ANALYSIS-003](docs/ANALYSIS-003_ModuleCrossVerification_v1.0.md)
...
| **빌드 도구** | `dotnet build` 또는 Visual Studio 2022 MSBuild |
| **RAM** | 최소 8GB (권장 16GB) |
| **디스크** | 최소 5GB |
...
**현재 상태 (2026-04-08 로컬 검증):**
- `dotnet build HnVue.sln -c Debug --no-restore` → 경고 0, 오류 0
- `dotnet test HnVue.sln --configuration Debug --nologo --no-restore -v minimal` → UI QA 테스트 15건 실패로 전체 실패
- 주요 실패 범주: `LoginView`, `PatientListView`, 디자인 토큰 배경색 PPT 정합성
...
# MSBuild / Coverlet 형식으로 수집
dotnet test --configuration Debug /p:CollectCoverage=true /p:CoverletOutputFormat=cobertura
```
...
- **경로:** `tests.integration/HnVue.IntegrationTests/`
...
`src/HnVue.UI.Contracts/`, `src/HnVue.UI.ViewModels/`는 현재 루트 README와 XML 문서 주석을 기준으로 구조를 설명합니다.
...
| MRD v4.0 | [`docs/planning/DOC-001_MRD_v3.0.md`](docs/planning/DOC-001_MRD_v3.0.md) | 시장 요구사항 문서 (v3.0 -> v4.0, 92개 MR) |
...
문서는 `planning`, `management`, `risk`, `verification`, `testing`, `regulatory`, `research`의 7개 주요 분류와 `docs/` 루트 기준 자료로 관리됩니다. 자동 동기화 스크립트 (`scripts/sync_docs.py`)로 버전 일관성을 유지합니다.
...
**운영 인프라 현황 (2026-04-08 구축 완료):**

| 항목 | 상태 | 비고 |
|------|:----:|------|
| Worktree 6개 | ✅ | `.worktrees/{team-name}/` (team/* 브랜치) |
| 이슈 레이블 | ✅ | Gitea 19개 + GitHub 19개 동기화 |
| 팀별 CLAUDE.md | ✅ | 각 worktree에 팀 전용 규칙 배치 |
| SonarCloud CI | ✅ | GitHub Actions 연동, 시크릿 설정 완료 |
| DOC-042 CMP | ✅ | Draft → Review, 도구 확정 |

**AI 개발 하네스 (2026-04-09 구축):**

| 항목 | 상태 | 비고 |
|------|:----:|------|
| HnVue 전용 에이전트 6개 | ✅ | `.claude/agents/hnvue/` — 팀 규칙 내장 |
| 오케스트레이터 스킬 | ✅ | `.claude/skills/hnvue-dev/` — 모듈 기반 자동 라우팅 |
| 도메인 레퍼런스 | ✅ | `hnvue-dev/references/domain-map.md` |
| CLAUDE.md 포인터 | ✅ | Section 18: Harness: HnVue Development |
...
> HnVue 모듈 관련 작업 요청 시 `hnvue-dev` 스킬이 자동 트리거되어 적절한 팀 에이전트로 라우팅됩니다.

| 문서 | 내용 |
|------|------|
| [운영 전략 가이드](docs/OPERATIONS.md) | 팀 구성, Worktree, 이슈 추적, QA/RA 기준 |
| [QA 릴리즈 기준](docs/OPERATIONS.md#5-qa-게이트-및-릴리즈-기준) | DOC-034 릴리즈 체크리스트, 자동화 파이프라인 |
| [RA 문서 절차](docs/OPERATIONS.md#6-ra-문서-유지보수-절차) | 규제 문서 업데이트 트리거 및 흐름 |
| [팀별 Context 가이드](docs/OPERATIONS.md#3-팀별-claude-context-가이드) | 각 팀 Claude 세션 최적화 설정 |
...
**프로젝트 상태:** Phase 1 코드 기반 완성 + 6팀 운영 인프라 + AI 개발 하네스 구축 (1,135개 테스트, 0.91/1.0) — 1차 릴리즈 준비도 ~82%  
**현재 단계:** Phase 1.5 — 6팀 분리 개발 체계 가동, AI 하네스 기반 자동 라우팅, Gap 분석 & 시험 보고서 현실화  
