...
CC가 팀에 "알림" 보내는 채널은 **없음**. 오직 git pull polling만 존재.
...
4. _CURRENT.md 재읽기 → 자기 팀 행 + ScheduleWakeup 값 재확인 (CC가 변경 가능)
...
- [HARD] ScheduleWakeup 없이 IDLE 보고 = CC와 단절 = 자율주행 불가
...
팀 세션 상태는 다음 3가지 git 신호로만 측정된다:

| 신호 | CC 해석 |
|------|---------|
| `origin/team/{team}` 최신 커밋 시각 | 팀 활동 시각 |
| DISPATCH Status 테이블의 타임스탬프 | Task 단위 진행 |
| `_CURRENT.md`의 팀 행 상태 | 라운드 단위 상태 (CC가 관리) |

팀이 ScheduleWakeup 없이 idle 상태면 CC는 팀 상태를 알 수 없음 → 반드시 재설정 필요.

---

## 5. Stall Detection [S09-R3 교훈]

CC는 모니터링 중 NOT_STARTED가 지속되는 팀을 감지:

- 3회 연속 NOT_STARTED → CC가 사용자에게 "작업 지연 의심" 경고
- 5회 연속 NOT_STARTED → CC가 사용자에게 조치 요청
- [HARD] CC는 **경고만** 수행, 팀 Status를 임의 변경 금지
- 사례: QA 12회 연속 NOT_STARTED → CC가 임의로 BLOCKED 변경 → 실제로는 QA 작업 중 → 상태 왜곡
...
```
N-1팀 MERGED + 1팀 미응답:
1. DISPATCH 발행 후 60분 경과 → CC가 TIMEOUT 선언
...
- [HARD] "사용자가 모니터링 지시할 때까지 대기" 금지 — CC 자율 판단
...
- [HARD] CC 세션과 팀 세션도 독립 — CC는 git 기반 감지만 수행

---

Version: 1.0.0 (team-common.md §3 일부 + §11 일부에서 분리)
Effective: 2026-04-22
Cross-ref: `dispatch-protocol.md`, `cc-operating-protocol.md`
