...
두 개 이상의 검사(Study) 영상을 병합하여 비교/분석할 수 있는 화면입니다. 의료진 워크플로우에서 영상 비교 기능은 필수적이며, 환자 데이터 오병합을 방지하는 안전 장치가 포함됩니다.
...
| 버튼 | 스타일 | 크기 |
|------|-------|------|
| 미리보기 | `HnVue.PrimaryButton` | Height 48px, icon 👁️ + 텍스트 |
