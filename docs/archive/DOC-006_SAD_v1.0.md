...
다음 다이어그램은 C4 Model Level 1 (System Context) 관점에서 HnVue Console SW와 외부 시스템/사용자의 상호작용을 나타낸다.

```mermaid
C4Context
    title HnVue Console SW - System Context Diagram （C4 Level 1）
    ...
    System(hnvue, "HnVue Console SW", "진단 X-Ray 촬영 제어, 영상 처리/표시, 선량 관리, DICOM 통신")
...
본 절은 Philippe Kruchten의 4+1 View Model을 기반으로 HnVue Console SW 아키텍처를 다섯 가지 관점에서 서술한다.
...
```mermaid
graph TD
    ROOT["HnVue Console SW<br/>（SW System）"]
