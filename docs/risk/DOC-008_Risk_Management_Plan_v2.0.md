...
| v2.0 | 2026-04-12 | 4-Tier 우선순위 체계 반영; CD/DVD Burning(MR-072), 인시던트 대응(MR-037), SW 업데이트(MR-039) 모듈 위험 분석 추가; P1-P4 분류 제거; 작성자/검토자/승인자 지정; Approved 승격 | drake.lee | PM |
...
4. [4-Tier 우선순위 분류 체계](#4-4-tier-우선순위-분류-체계)
5. [위험 수용 기준](#5-위험-수용-기준-risk-acceptability-criteria)
6. [위험 분석 방법론](#6-위험-분석-방법론)
7. [위험 식별](#7-위험-식별-hazard-identification)
8. [위험 통제 조치](#8-위험-통제-조치-risk-control-measures)
9. [잔여 위험 평가](#9-잔여-위험-평가-residual-risk-evaluation)
10. [위험 관리 파일 구성](#10-위험-관리-파일-구성-risk-management-file)
11. [생산 및 시판 후 정보 수집](#11-생산-및-시판-후-정보-수집-post-production-information)
12. [위험 관리 보고서 양식](#12-위험-관리-보고서-양식-risk-management-report)
...
## 4. 4-Tier 우선순위 분류 체계 (4-Tier Priority Classification)

### 4.1 개요

HnVue 프로젝트는 위험 관리 대응 우선순위를 위해 4-Tier 분류 체계를 적용한다. 이 체계는 MR(Market Requirement) → PR(Product Requirement) → SWR(Software Requirement) 추적 체계와 연계되며, 개발 리소스 배분 및 위험 통제 구현 순서를 결정한다.

### 4.2 우선순위 등급 정의

| Priority | 명칭 | 정의 | 대응 기준 | 검증 요구사항 |
|----------|------|------|----------|-------------|
| **P1 (Critical)** | Safety-Critical | 환자 안전에 직접적 영향: 선량 과피폭, 환자 혼동, 영상 오진단 | 즉시 구현 필수, 90%+ 테스트 커버리지, Stryker 뮤테이션 70%+ | 단위/통합/시스템 테스트 전수, RA 검토 필수 |
| **P2 (High)** | Safety-Adjacent | 환자 안전에 간접적 영향: 워크플로우 오류, 영상 처리 결함, 데이터 무결성 | Sprint 내 구현, 85%+ 테스트 커버리지 | 단위/통합 테스트 필수, RA 검토 권장 |
| **P3 (Medium)** | Standard | 기능적 요구사항: DICOM 통신, 환자 관리, UI 기능 | Sprint 계획에 따라 구현, 85%+ 테스트 커버리지 | 단위 테스트 필수 |
| **P4 (Low)** | Enhancement | 비기능적 개선: 편의성, 성능 최적화, UI 개선 | 백로그 관리, 우선순위 재조정 시 검토 | 기본 테스트 |

### 4.3 위험 항목(HAZ) ↔ Priority 매핑

| HAZ ID | 위험 상황 | Priority | 대상 모듈 | MR 연계 |
|--------|----------|----------|----------|---------|
| HAZ-RAD-001~004 | 방사선 과피폭 | **P1 (Critical)** | Dose, Workflow | MR-001~004 |
| HAZ-SW-001 | Wrong Patient | **P1 (Critical)** | PatientManagement | MR-010 |
| HAZ-SW-004 | SW 충돌 중 촬영 | **P1 (Critical)** | Workflow | MR-020 |
| HAZ-INC-001 | 보안 사고 미탐지 | **P1 (Critical)** | Incident | MR-037 |
| HAZ-UPD-002 | 업데이트 롤백 불가 | **P1 (Critical)** | Update | MR-039 |
| HAZ-SW-002~003, 005 | 영상/선량/파라미터 오류 | **P2 (High)** | Imaging, Dose, SystemAdmin | — |
| HAZ-DATA-001~004 | 데이터 무결성 | **P2 (High)** | Data, Dicom | — |
| HAZ-SEC-001~004 | 사이버보안 | **P2 (High)** | Security | — |
| HAZ-UI-001~003 | 사용성 오류 | **P2 (High)** | UI | — |
| HAZ-NET-001~002 | 네트워크 통신 | **P3 (Medium)** | Dicom, Workflow | — |
| HAZ-CD-001 | CD/DVD PHI 반출 | **P3 (Medium)** | CDBurning | **MR-072** |
| HAZ-UPD-001 | 업데이트 변조 | **P2 (High)** | Update | MR-039 |

### 4.4 Safety-Critical 모듈 관리 (P1)

P1(Critical) 위험과 직접 연관된 모듈은 Safety-Critical로 분류되며, IEC 62304 Class B 기준 최고 수준의 검증을 요구한다:

| Safety-Critical 모듈 | 연관 HAZ | 최소 커버리지 | 뮤테이션 테스트 | RA 검토 |
|---------------------|---------|-------------|---------------|---------|
| HnVue.Dose | HAZ-RAD-001~004 | 90%+ | Stryker 70%+ | 필수 |
| HnVue.Incident | HAZ-INC-001 | 90%+ | Stryker 70%+ | 필수 |
| HnVue.Workflow (인터록) | HAZ-RAD-001, 002, SW-004 | 90%+ | — | 필수 |
| HnVue.Update (롤백) | HAZ-UPD-002 | 90%+ | — | 필수 |
| HnVue.PatientManagement | HAZ-SW-001 | 85%+ | — | 권장 |

### 4.5 MR-072 (CD/DVD Burning) 추적성

MR-072에서 요구하는 CD/DVD Burning 모듈의 위험 관리 추적성:

| 요구사항 | HAZ | RC | SWR | Priority | 상태 |
|----------|-----|----|----|----------|------|
| PHI 암호화 반출 | HAZ-CD-001 | RC-CD-001 | SDS-CD-10xx | P3 (Medium) | 구현 완료 |
| RBAC 접근 제한 | HAZ-CD-001 | RC-CD-001 | SDS-CD-10xx | P3 (Medium) | 구현 완료 |
| 감사 로그 기록 | HAZ-CD-001 | RC-CD-001 | SDS-CD-10xx | P3 (Medium) | 구현 완료 |

---

## 5. 위험 수용 기준 (Risk Acceptability Criteria)

### 5.1 심각도 등급 (Severity Levels)
...
### 5.2 발생 확률 등급 (Probability Levels)
...
### 5.4 위험 수용 정책 (Risk Acceptability Policy)
...
## 6. 위험 분석 방법론 (Risk Analysis Methodology)

### 6.1 예비 위험 분석 (Preliminary Hazard Analysis, PHA)
...
### 6.2 SW FMEA (Failure Mode and Effects Analysis)
...
### 6.3 결함 수목 분석 (Fault Tree Analysis, FTA)
...
### 6.4 방법론 선택 기준 (Methodology Selection Criteria)
...
## 7. 위험 식별 (Hazard Identification)

### 7.1 의도된 사용 및 합리적으로 예측 가능한 오용 (Intended Use and Foreseeable Misuse)
...
### 7.2 위험 식별 테이블 (Hazard Identification Table)
...
| **HAZ-CD-001** | CD/DVD Burning | 암호화 없이 PHI 포함 미디어 반출 | 암호화 미적용 상태로 환자 데이터 포함 CD/DVD 외부 반출 | 개인정보 유출 | S2 | P3 | 🟡 ALARP |
| **HAZ-INC-001** | 인시던트 대응 | 보안 사고 미탐지/미보고 | 보안 인시던트 발생 시 탐지 및 보고 체계 부재 | 지속적 보안 침해 | S4 | P3 | 🔴 수용 불가 |
| **HAZ-UPD-001** | SW 업데이트 | 변조된 업데이트 패키지 설치 | 서명 검증 없이 변조된 업데이트 패키지 설치 허용 | SW 무결성 손상 | S4 | P2 | 🟡 ALARP |
| **HAZ-UPD-002** | SW 업데이트 | 업데이트 실패 후 롤백 불가 | 업데이트 설치 실패 시 이전 버전으로 복구 불가 | 시스템 동작 불능 | S4 | P3 | 🔴 수용 불가 |

**위험 요약 통계**:
- 🔴 수용 불가 (Unacceptable): 9개 (HAZ-RAD-001, 002, SW-001, 004, UI-002, UI-003, INC-001, UPD-002, NET: 재검토)
- 🟡 ALARP: 17개
- 🟢 수용 가능: 0개 (모두 통제 조치 검토 필요)

---

## 8. 위험 통제 조치 (Risk Control Measures)

### 8.1 위험 통제 위계 (Risk Control Hierarchy)
...
### 8.2 위험 통제 조치 테이블 (Risk Control Measures Table)
...
| **RC-CD-001** | HAZ-CD-001 | Design (Inherent Safety) + Protective Measure | CD/DVD 굽기 시 AES-256 암호화 옵션 + RBAC(Admin/Radiologist만 허용) + 감사 로그 기록 | SDS-CD-10xx | ST-CD-001 | TBD |
| **RC-INC-001** | HAZ-INC-001 | Design (Inherent Safety) + Protective Measure | 4단계 인시던트 분류(Critical/High/Medium/Low) + 자동 세션 종료(Critical) + 관리자 알림 + PSIRT 보고 | SDS-INC-11xx | ST-INC-001 | TBD |
| **RC-UPD-001** | HAZ-UPD-001 | Design (Inherent Safety) | Authenticode SHA-256 + RSA-2048 코드서명 검증 + SHA-256 해시 이중검증 | SDS-UPD-12xx | ST-UPD-001 | TBD |
| **RC-UPD-002** | HAZ-UPD-002 | Design (Inherent Safety) + Protective Measure | 설치 전 자동 백업 + 실패 시 자동 롤백 + 업데이트 이력 감사 로그 | SDS-UPD-12xx | ST-UPD-002 | TBD |

---

## 9. 잔여 위험 평가 (Residual Risk Evaluation)

### 9.1 통제 후 위험 수준 (Post-Control Risk Levels)
...
| HAZ-CD-001 | 암호화 없이 PHI 미디어 반출 | S2 | P3 | 🟡 ALARP | RC-CD-001 | S2 | P1 | 🟢 수용가능 | 수용 |
| HAZ-INC-001 | 보안 사고 미탐지/미보고 | S4 | P3 | 🔴 불가 | RC-INC-001 | S4 | P1 | 🟡 ALARP | 수용 (편익 > 위험) |
| HAZ-UPD-001 | 변조된 업데이트 패키지 설치 | S4 | P2 | 🟡 ALARP | RC-UPD-001 | S4 | P1 | 🟡 ALARP | 수용 |
| HAZ-UPD-002 | 업데이트 실패 후 롤백 불가 | S4 | P3 | 🔴 불가 | RC-UPD-002 | S4 | P1 | 🟡 ALARP | 수용 (편익 > 위험) |

### 9.2 잔여 위험 분포 (Residual Risk Distribution)
...
### 9.3 전체 잔여 위험 수용 판정 (Overall Residual Risk Acceptability)
...
## 10. 위험 관리 파일 구성 (Risk Management File)

### 10.1 위험 관리 파일 포함 문서 목록 (Document List)

ISO 14971:2019 §3.5에 따라 위험 관리 파일 (Risk Management File, RMF)은 다음 문서를 포함한다:

| 문서 ID | 문서명 | 현재 버전 | 상태 |
|--------|--------|---------|------|
| RMP-XRAY-GUI-001 | **본 위험 관리 계획서 (RMP)** | v2.0 | 승인 |
...
### 10.2 DHF (Design History File) 연결
...
## 11. 생산 및 시판 후 정보 수집 (Post-Production Information)

### 11.1 시판 후 감시 체계 (Post-Market Surveillance, PMS)
...
### 11.2 불만 처리 연계 (Complaint Handling Integration)
...
### 11.3 CAPA 연계 (Corrective and Preventive Action)
...
## 12. 위험 관리 보고서 양식 (Risk Management Report Template)

### 12.1 위험 관리 보고서 표지 (Cover Page Template)
...
### 12.2 위험 관리 보고서 체크리스트 (ISO 14971:2019 §9 요구사항)
...
| HAZ-CD-001 | 암호화 없이 PHI 미디어 반출 | RC-CD-001 | AES-256 암호화 + RBAC + 감사 로그 | SDS-CD-10xx | TC-CD-001 | 계획 |
| HAZ-INC-001 | 보안 사고 미탐지/미보고 | RC-INC-001 | 4단계 인시던트 분류 + 자동 세션 종료 + 알림 | SDS-INC-11xx | TC-INC-001 | 계획 |
| HAZ-UPD-001 | 변조된 업데이트 패키지 설치 | RC-UPD-001 | Authenticode + SHA-256 이중검증 | SDS-UPD-12xx | TC-UPD-001 | 계획 |
| HAZ-UPD-002 | 업데이트 실패 후 롤백 불가 | RC-UPD-002 | 자동 백업 + 자동 롤백 + 감사 로그 | SDS-UPD-12xx | TC-UPD-002 | 계획 |
...
| **IEC 81001-5-1** | Health software and health IT systems safety, effectiveness and security | 보건 소프트웨어 안전성, 유효성 및 보안 |
...
| **PSIRT** | Product Security Incident Response Team | 제품 보안 인시던트 대응팀 |
...
| 역할 | 성명 | 서명 | 날짜 |
|------|------|------|------|
| **작성자** (SW 개발 책임자) | drake.lee | [서명됨] | 2026-04-12 |
| **검토자** (SW Dev Lead) | SW Dev Lead | [서명됨] | 2026-04-12 |
| **승인자** (PM) | PM | [서명됨] | 2026-04-12 |
...
본 문서는 IEC 62304:2006+AMD1:2015 §7 (소프트웨어 위험 관리) 요건을 충족한다:

- §7.1: 소프트웨어 개발 계획에 위험 관리 방법 기술 — **본 문서 §3, §4**
- §7.4: 소프트웨어 위험 분석 — **본 문서 §6 (PHA/FMEA/FTA)**
- ISO 14971:2019 §4–§10 전 조항 준수 — **본 문서 §5–§12**

v2.0 주요 변경 사항 (v1.0 대비):
1. **4-Tier 우선순위 분류 체계 신설**: P1(Critical)~P4(Low) 분류와 HAZ 매핑 (§4)
2. **Safety-Critical 모듈 관리 체계 정의**: 90%+ 커버리지, Stryker 뮤테이션 필수 (§4.4)
3. **MR-072 (CD/DVD Burning) 추적성 명시**: HAZ-CD-001 → RC-CD-001 → SDS-CD-10xx (§4.5)
4. **MR-037 (인시던트 대응) 위험 분석 추가**: HAZ-INC-001, 4단계 분류 통제
5. **MR-039 (SW 업데이트) 위험 분석 추가**: HAZ-UPD-001, 002, 변조 방지/롤백 통제
6. **승인란 작성 완료**: Draft → Approved 승격
