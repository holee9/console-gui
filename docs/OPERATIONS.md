...
# HnVue AI 에이전트 중심 워크트리 개발 운영 전략

## 개요

이 문서는 HnVue 프로젝트의 AI 에이전트 중심 멀티팀 워크트리 개발 운영 전략을 정의합니다.

> **핵심 원칙: AI 에이전트가 구현하고, 인간이 리뷰/의사결정한다.**

AI 에이전트 6팀이 병렬로 코드를 구현하며, 인간 7명은 PR 리뷰, 아키텍처 결정, 도메인 검증, 규제 승인 등 의사결정에 집중합니다. 코딩 속도가 아닌 인간 리뷰 속도와 외부 의존성이 일정의 실질적 병목입니다.

**대상 독자:** 인간 리뷰어 (팀 리더, 도메인 전문가, QA, RA)

---

## 1. 팀 구성 및 역할

### 실행 모델: AI 에이전트 구현 + 인간 리뷰

| 구분 | 주체 | 행동 |
|------|------|------|
| **구현 실행** | AI 에이전트 6팀 (MoAI 오케스트레이션) | 코딩, 테스트 작성, 문서 생성, 빌드 검증 |
| **리뷰/승인** | 인간 7명 (도메인별 리뷰어) | PR 리뷰, 아키텍처 결정, Safety 검증, 인허가 서명 |
| **외부 조달** | 인간 (CC/팀장) | HW 확보, 벤더 SDK 계약, 침투테스트 발주, 사용성 테스트 섭외 |

### 인간-AI 역할 매핑 (7명 인간 + AI 6팀)

| 인간 역할 | 인원 | AI 에이전트 | 인간이 하는 일 | AI가 하는 일 |
|-----------|------|-----------|--------------|------------|
| **CC (총괄)** | 1명 | MoAI 오케스트레이터 | Sprint 계획 승인, DISPATCH 발행 지시 | DISPATCH 작성, 팀 조율, 진도 추적 |
| **SW 팀장** | 1명 | hnvue-coordinator + hnvue-infra | 아키텍처 결정, PR 리뷰/머지 승인 | DI 통합, ViewModel, 인프라 구현 |
...
| **RA** | 1명 | hnvue-ra | 규제 문서 최종 승인, 인허가 제출 | RTM/SBOM/CMP 문서 생성, 추적성 |

**Commander Center는 코드를 작성하지 않음** — 계획/지시/검증만 수행
**AI 에이전트는 의사결정을 하지 않음** — 구현/생성/분석만 수행, 판정은 인간에게 위임

### 팀 구조 정의

HnVue 프로젝트는 6개 워크트리 팀으로 구성되며, 각 팀은 특정 모듈 영역과 책임을 담당합니다.

#### Team A — 인프라 및 기초 계층 (SW 개발팀장 겸임 + 개발자 2)
...
#### Team B — 의료 영상 처리 파이프라인 (개발자 1 전담)
...
#### Team Design — 순수 UI 디자인 (디자이너 전담, 코더 없음)
...
#### Coordinator — 통합 및 UI 계약 관리 (SW 개발팀장 겸임)
...
**테스트 책임:**
- **정적/동적 분석:** team-common.md Quality Standards 참조 (SINGLE SOURCE OF TRUTH)
- **보안 분석:** OWASP 스캔 (CVSS>=7.0 빌드 실패)
...
| 활동 | Team A (팀장+Dev2) | Team B (Dev1) | Design (디자이너) | Coordinator (팀장) | QA | RA |
|------|--------|--------|--------|------------|----|----|
| 공통 인터페이스 변경 | **R** | C | - | **A** | I | I |
| DB 마이그레이션 | **R/A** | I | - | C | C | C |
| UI.Contracts 변경 | C | C | I | **R/A** | I | I |
| ViewModel 구현 | - | I | - | **R/A** | - | - |
| XAML View 코드화 | - | - | **R** | **A** | I | - |
| 도메인 Converter (C#) | - | **R** | C (색상 제안) | I | I | - |
| Medical Component (C#) | - | **R** | C (XAML 템플릿) | I | I | - |
...
**겸임 참고:** Team A와 Coordinator는 SW 개발팀장 1명이 겸임. Design은 디자이너 전담. 개발자 2는 Team A 보조.
...
HnVue 프로젝트는 7개 워크트리(main + 6개 팀)를 사용하여 팀별 병렬 개발을 지원합니다:

```
Console-GUI/
├── (main 브랜치 — 릴리즈 통합)
└── .worktrees/
    ├── coordinator/    ← team/coordinator 브랜치
    ├── qa/             ← team/qa 브랜치
    ├── ra/             ← team/ra 브랜치
    ├── team-a/         ← team/team-a 브랜치
    ├── team-b/         ← team/team-b 브랜치
    └── team-design/    ← team/team-design 브랜치
```

각 워크트리는 독립적인 `team/*` 브랜치에 연결되며, 팀 간 파일 충돌을 방지합니다.
각 워크트리에는 팀 전용 `CLAUDE.md`와 `.claude/rules/team-context.md`가 배치되어 있습니다.

### 워크트리 생성 명령어

**초기 설정 (프로젝트 리더만 실행 — 2026-04-08 완료):**

```bash
cd D:\workspace-gitea\Console-GUI

# 6개 팀 워크트리 일괄 생성
for team in coordinator qa ra team-a team-b team-design; do
  git branch "team/$team" main
  git worktree add ".worktrees/$team" "team/$team"
done
```
...
```powershell
# 전체 팀 초기화
.\scripts\team\Init-TeamWorktree.ps1

# 특정 팀만 초기화
.\scripts\team\Init-TeamWorktree.ps1 -Team team-a
```
...
```bash
# 손상된 워크트리 제거
git worktree remove .worktrees/team-a
