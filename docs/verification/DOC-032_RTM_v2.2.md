...
| v2.8 | 2026-04-19 | RA팀 | S13-R1: STRIDE 보안 통제, Print SCU, PACS 비동기, RDSR 강화 SWR 추적성 추가 (부록 H). M1 Gate 평가 DOC-047 v1.1 연계. |
...
## 부록 H. S13-R1 STRIDE 보안 통제 추적성

> **상태**: PENDING — Team A S13-R1 구현 완료 후 TC ID 확정
>
> **배경**: S13-R1에서 Team A가 STRIDE 보안 통제(Tier 1)를 구현. PHI AES-256-GCM 암호화, TLS 1.3 기초, 보안 인증 강화 포함.
> M1 Gate (2026-05-15)를 위한 STRIDE 검토 평가는 DOC-047 v1.1 부록 A에 문서화.

### H.1 STRIDE 보안 통제 → SWR 매핑

| 통제 번호 | 통제명 | 관련 DOC-017 위협 | 기존 SWR | S13-R1 신규/변경 SWR | 구현 팀 | TC ID | 상태 |
|-----------|--------|-------------------|---------|---------------------|---------|-------|------|
| 1 | Authentication | TM-S-001~006 | SWR-CS-070~072 | SWR-CS-070a (bcrypt 12 rounds 강화) | Team A | TBD | PENDING |
| 3 | Cryptography | TM-I-001~006 | SWR-CS-078~081 | SWR-CS-080a (PHI AES-256-GCM 구현) | Team A | TBD | PENDING |
| 3 | Cryptography | TM-T-001 | SWR-DC-063~064 | SWR-DC-063a (TLS 1.3 기초 설정) | Team A | TBD | PENDING |
| 5 | Confidentiality | TM-I-001~005 | SWR-CS-080~081 | SWR-CS-080b (저장 데이터 암호화 강화) | Team A | TBD | PENDING |

### H.2 Print SCU → SWR 매핑

| SWR ID | 요구사항명 | 상위 PR | 상위 MR | 구현 모듈 | TC ID | 상태 |
|--------|-----------|---------|---------|----------|-------|------|
| SWR-DC-059 | DICOM Print SCU 기본 | PR-DC-054 (Print SCU, Tier 2) | MR-005 (DICOM, Tier 1) | HnVue.Dicom (Team B) | TBD | PENDING |
| SWR-DC-060 | Print SCU 필름 레이아웃 | PR-DC-054 | MR-005 | HnVue.Dicom (Team B) | TBD | PENDING |

### H.3 PACS 비동기 파이프라인 → SWR 매핑

| SWR ID | 요구사항명 | 상위 PR | 상위 MR | 구현 모듈 | TC ID | 상태 |
|--------|-----------|---------|---------|----------|-------|------|
| SWR-DC-065 | PACS 비동기 전송 큐 관리 | PR-DC-050 (Storage SCU, Tier 1) | MR-005 (DICOM, Tier 1) | HnVue.Dicom (Team B) | TBD | PENDING |
| SWR-DC-066 | PACS 전송 재시도 로직 | PR-DC-050 | MR-005 | HnVue.Dicom (Team B) | TBD | PENDING |

### H.4 RDSR 생성 강화 → SWR 매핑

| SWR ID | 요구사항명 | 상위 PR | 상위 MR | 구현 모듈 | TC ID | 상태 |
|--------|-----------|---------|---------|----------|-------|------|
| SWR-DM-044a | RDSR 생성 강화 (DICOM Radiaton Dose SR) | PR-DM-042 (RDSR 생성, Tier 2) | MR-004 (선량 관리, Tier 2) | HnVue.Dose (Team B) | TBD | PENDING |

### H.5 S13-R1 추적성 요약

| 카테고리 | SWR 수 | TC 수 | 매핑 상태 |
|----------|--------|-------|----------|
| STRIDE 보안 통제 | 4 | TBD | **PENDING** (Team A 구현 완료 후) |
| Print SCU | 2 | TBD | **PENDING** (Team B 구현 완료 후) |
| PACS 비동기 | 2 | TBD | **PENDING** (Team B 구현 완료 후) |
| RDSR 강화 | 1 | TBD | **PENDING** (Team B 구현 완료 후) |
| **합계** | **9** | **TBD** | **0% (구현 진행 중)** |

> **갱신 예정**: S13-R1 COMPLETED 후 Team A/B TC ID 확정 시 본 부록 갱신. xUnit Trait 기반 자동 추적성 연계.
