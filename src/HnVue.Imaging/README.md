...
| SWR ID | 대상 메서드 | 내용 |
|--------|-------------|------|
| SWR-IP-020 | `ProcessAsync` | DICOM/Raw 파일 로드 및 픽셀 추출 |
| SWR-IP-022 | `ApplyWindowLevel`, `Zoom` | 창/레벨 LUT 적용 |
| SWR-IP-024 | `Zoom` | 업스케일 Bicubic, 다운스케일 Area Average |
| SWR-IP-026 | `Pan` | PanOffsetX/Y 누적, 픽셀 데이터 불변 |
| SWR-IP-027 | `Rotate`, `Flip` | 90/180/270° 회전, 수평/수직 반전 |
| SWR-IP-036 | `ProcessAsync` | ROI 통계용 RawPixelData16 보존 (0~65535) |
| SWR-IP-052 | `ProcessAsync` | 밝기 오프셋 연산용 16비트 원본 버퍼 보존 |
