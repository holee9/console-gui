...
/// Performs a C-FIND query against the configured Modality Worklist SCP.
...
{
    private readonly DicomFindScu _findScu;
    private readonly IDicomNetworkConfig _config;

    /// <summary>
    /// Initialises a new <see cref="WorklistRepository"/>.
    /// </summary>
    /// <param name="findScu">DICOM C-FIND SCU used for MWL queries.</param>
    /// <param name="config">DICOM network configuration providing MWL endpoint details.</param>
    public WorklistRepository(DicomFindScu findScu, IDicomNetworkConfig config)
    {
        _findScu = findScu ?? throw new ArgumentNullException(nameof(findScu));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var queryResult = await _findScu
