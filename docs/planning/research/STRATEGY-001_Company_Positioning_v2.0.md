...
**2명의 SW 인력으로 경쟁사 수준의 풀 Console을 자체 개발하는 것은 비현실적이다.** 그러나 현재 구매 중인 타사 SW의 핵심 기능을 대체하는 수준의 자체 SW는 약 24-36 man-month(SW 2명 기준) 규모로 추정된다.
...
### 4.3 Phase 1: 타사 SW 대체 (~24-36 man-month, SW 2명 기준)
...
### 4.4 Phase 2: 업계 표준 달성 (~18-24 man-month, 인력 보강 전제)
...
| 영역 | 투입 시점 | 예상 비용 |
|------|----------|----------|
| IEC 62304 문서화/컨설팅 | Phase 1 전체 | 30-50M KRW |
| UI/UX 디자인 | Phase 1 초~중반 | 10-20M KRW |
| QA/테스트 (V&V) | Phase 1 중~후반 | 15-25M KRW |
| MFDS 인허가 대행 | Phase 1 후반 | 20-30M KRW |
| PACS 호환 테스트 | Phase 1 중반 | 5-10M KRW |
| **소계** | | **80-135M KRW** |
...
| 역할 | 시기 | 우선순위 | 근거 |
|------|------|----------|------|
| SW 개발자 1명 추가 | Phase 1 초반 | **필수** | 2명은 SPOF. 1명 이탈 시 프로젝트 중단 |
| QA 엔지니어 | Phase 1 중반 | 높음 | IEC 62304 V&V 내부 수행 |

---

## 6. 공수 추정 및 마일스톤

### 6.1 Phase 1 마일스톤 (~24-36 MM)

| 마일스톤 | 예상 공수 | 산출물 | 핵심 지표 |
|----------|----------|--------|----------|
| **MS-01 기반 구축** | ~8-10 MM | DICOM 인프라(fo-dicom), 기본 이미지 표시, DB 스키마, 아키텍처 문서 | DICOM Echo 성공, 이미지 로드/표시 |
| **MS-02 촬영 통합** | ~8-10 MM | 디텍터+Generator 연동, 촬영 워크플로우, MWL, C-STORE | 자사 FPD로 촬영→PACS 전송 성공 |
| **MS-03 임상 기능** | ~4-8 MM | 영상 처리 프리셋(SDK 연동), 어노테이션, 스터디 관리, 선량 기록 | 임상 워크플로우 end-to-end 완료 |
| **MS-04 규제/출시** | ~4-8 MM | IEC 62304 문서, V&V, MFDS 신청 | MFDS 접수 |

> **MM = man-month.** SW 2명 기준. 3명 확보 시 공수 불변, 기간 단축.

### 6.2 역할별 공수 배분

| 역할 | MS-01 | MS-02 | MS-03 | MS-04 |
|------|-------|-------|-------|-------|
| Dev A (시니어) | DICOM/아키텍처 | 디텍터+Generator 통합 | 영상처리 SDK 연동 | V&V/버그수정 |
| Dev B | DB/UI 프레임워크 | 워크플로우 UI | 스터디 관리/UX | V&V/버그수정 |
| 외주 | IEC 62304 컨설팅 시작 | UI/UX 디자인 | QA 테스트 | MFDS 인허가 |
...
| IEC 62304 경험 부족 | 높음 | 높음 | **높음** | Phase 1 착수 즉시 외부 컨설팅 |
| 일정 지연 (공수 초과) | 높음 | 중 | **높음** | Phase 1 범위 추가 축소 (15→12개 기능) |
...
> MRD(DOC-001)에서는 제품 성능 지표만 관리. 아래 내재화 프로젝트 지표는 본 전략 문서에서 관리한다.

### 9.1 Phase 1 내재화 지표

| KPI | Phase 1 목표 | 측정 방법 |
|-----|-------------|---------|
| feel-DRCS 기능 대체율 | 100% (기본 기능) | 기능 체크리스트 대조 |
| 자사 FPD 번들 테스트 통과 | 3개 모델 이상 | 호환성 테스트 리포트 |
| MFDS 인허가 접수 | Phase 1 종료 시 | 접수 완료 여부 |
| 외부 SW 라이선스 비용 절감 | feel-DRCS 라이선스 종료 | 연간 비용 비교 |
| IEC 62304 문서 완비 | Class B 전체 | DHF 감사 |
| Generator 연동 검증 | 자사 Generator 2개+ | 통합 테스트 리포트 |

### 9.2 Phase 2 지표

| KPI | Phase 2 목표 |
|-----|-------------|
| 영상처리 엔진 내재화율 | 100% (외부 SDK 의존 제거) |
| 멀티벤더 디텍터 지원 | 5개+ 제조사 |
| FDA 510(k) 접수 | Phase 2 종료 시 |
| Reject Rate 감소 | >= 15% |
| 선량 DRL 초과율 | <= 5% |
...
| # | 의사결정 | 옵션 | 권장 | 근거 |
|---|---------|------|------|------|
| D-01 | 3번째 SW 개발자 채용 | 즉시/Phase 1 초반/미채용 | **즉시** | 2명은 SPOF |
| D-02 | IEC 62304 컨설팅 계약 | 즉시/Phase 1 중반 | **즉시** | 개발 초기부터 병행 필수 |
| D-03 | feel-DRCS 라이선스 유지 기간 | Phase 1 완료까지/Phase 2까지 | **Phase 1 완료+안정화까지** | 자체 HnVue SW 안정화 전까지 판매용 유지 |
...
| # | 의사결정 | 시점 | 판단 기준 |
|---|---------|------|----------|
| D-06 | Phase 1 범위 조정 여부 | MS-02 완료 시점 | 공수 초과 시 15→12개 기능으로 축소 |
| D-07 | QA 내부화 vs 외주 | MS-03 착수 시점 | V&V 볼륨에 따라 |
| D-08 | MFDS 우선 vs FDA 우선 | MS-02 중 | MFDS가 비용/기간 효율적 |
...
> 다음 업데이트: Phase 1 착수 후 MS-01 중간 시점 (D-01–D-03 의사결정 반영)
