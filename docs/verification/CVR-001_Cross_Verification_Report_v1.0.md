...
- MRD에서 정의된 MR-001–MR-062는 PRD v2.0의 "출처 MR" 컬럼 및 RTM에 올바르게 참조됨
- 단, PRD v2.0의 표지 정보(§1.1)에 **"MRD 참조: MRD-XRAY-001 v1.2"** 라고 기재되어 있으나, 실제 MRD 문서 ID는 **MRD-XRAY-GUI-001 v2.0**임 → **문서 ID 불일치**
...
- PRD v2.0의 PR-PM-002 요구사항 테이블에는 파생 SWR로 **"SWR-PM-004, SWR-PM-005"** 가 명시됨
...
- FRS는 PRD v2.0의 "v3.0 3단계 계층 ID 체계" 도입에 따라 SWR 번호가 40, 50, 70번대 등 점프된 번호를 사용
...
| 발견 ID | 심각도 | 발견 위치 | 내용 |
|---------|--------|----------|------|
| F-08 | 🟠 Major | PRD §3(PR-PM-003, PR-PM-006), FRS §3 | PRD에 명시된 파생 SWR(SWR-PM-006–008, SWR-PM-013–014)이 FRS 실제 SWR(SWR-PM-020–024, SWR-PM-050–053)과 불일치. PRD의 "파생 SWR" 컬럼 전반에 걸쳐 FRS v2.0 기준으로 재검토 필요. |
...
WBS v1.0의 문서 계획(Gantt 및 DOC 참조)은 DMP v2.0의 문서 목록과 대체로 일치한다. WBS는 DOC-001–036 번호 체계를 참조하며 DMP와 동일 체계를 사용한다.
...
**결론**: RC-001–036 중 FRS와 정확히 일치하는 SWR을 참조하는 RC는 소수에 불과하며, 대다수 RC가 FRS에 존재하지 않는 SWR을 참조한다. RMP가 이전 버전 SWR 체계를 기반으로 작성되어 FRS v2.0 체계로 업데이트되지 않은 것이 원인이다.
...
PRD v2.0의 Appendix A(RTM)에서 MR-001 → PR-PM-002 → SWR-PM-004–005 매핑이 명시되어 있으나, FRS에서 SWR-PM-005는 정의되어 있지 않다 (SWR-PM-004 다음 SWR-PM-006부터 시작). PR-PM-002는 FRS에서 SWR-PM-010–013으로 분화되어 있다.
...
**권고 R-03**: FRS v2.0 기준으로 RMP 전체 SWR 참조 재수정
- RC 테이블의 SWR 참조를 FRS v2.0 번호 체계(SWR-DM-040번대, SWR-CS-070번대, SWR-SA-060번대 등)로 전면 업데이트
- 특히 FRS에 미정의된 SWR-UI-xxx 도메인 처리: FRS에 SWR-UI 섹션 신설하거나 해당 기능을 PR-WF/PR-UX 등에 통합
- **근거**: F-17 / **담당**: SE팀 + QA팀 / **기한**: 2주 이내

**권고 R-04**: QA Plan을 FRS v2.0 SWR 번호 체계로 전면 개정
...
**권고 R-06**: PRD 파생 SWR 컬럼을 FRS v2.0 기준으로 전면 재검토
