...
| `JwtOptions` | `sealed class` | 호스트 앱이 구성해 `AddHnVueSecurity()`에 전달하는 JWT 옵션. `SecretKey` 최소 32자 강제 |
...
- `ITokenDenylist` → `InMemoryTokenDenylist` (Singleton)
...
| 파일 | 목적 | git 추적 |
|------|------|----------|
| `src/HnVue.App/appsettings.json` | 기본/개발용 플레이스홀더 | ✅ 추적 |
| `src/HnVue.App/appsettings.Development.json` | 로컬 개발용 플레이스홀더 | ✅ 추적 |
| 환경변수 또는 Secret Store | 배포/CI 오버라이드 | N/A |

### 필수 설정 키

| 설정 키 | 환경변수 | 설명 |
|---------|---------|------|
| `Security:JwtSecretKey` | `HNVUE_JWT_SECRET` | HMAC-SHA256 JWT 서명 키 (최소 32자) |
...
프로덕션 환경에서는 환경변수 또는 별도 secret store 사용이 권장됩니다.
...
| 테스트 파일 | `AuditServiceTests.cs`, `JwtTokenServiceTests.cs`, `PasswordHasherTests.cs`, `RbacPolicyTests.cs`, `SecurityServiceTests.cs`, `TokenDenylistTests.cs` |
| 검증 범위 | 인증/인가, BCrypt 해싱, RBAC, 감사 로그 체인, Quick PIN, 로그아웃 토큰 폐기 |
...
#### Step 1: `LogoutAsync(userId, ...)` 호출
```
SecurityService.LogoutAsync(userId)
  ↓
ISecurityContext.CurrentJti 확인
  ↓
...
```
JwtTokenService.Validate(token, tokenDenylist)
  ↓
