...
MRD v3.0의 4-Tier 분류 체계를 계승합니다. 벤치마크: FDA K231225 (DRTECH EConsole1), feel-DRCS (K110033), MFDS 2024 가이드라인.

| Tier | 의미 | 기준 | Phase |
|------|------|------|-------|
| **Tier 1** | 인허가 필수 | MFDS/FDA/IEC 필수 | Phase 1 |
| **Tier 2** | 시장 진입 필수 | feel-DRCS 동등 + 고객 최소 기대 | Phase 1 |
| **Tier 3** | 있으면 좋고 | EConsole1 K231225 미포함, 차별화 | Phase 2+ |
...
| **Tier** | Tier 1 / Tier 2 / Tier 3 / Tier 4 (MRD v3.0 4-Tier 체계) |
| **Phase** | 구현 대상 Phase (1 / 2 / 3+) |
...
3. **규제 준수**: FDA 510(k) / CE MDR / MFDS 인허가 획득
...
| 촬영 워크플로우 | APR 관리, 촬영 순서 제어, 기기 통신, CD/DVD Burning |
| 영상 표시·처리 | 실시간 영상 표시, 기본 영상 처리 파이프라인, 뷰어 도구 |
| 선량 관리 | DAP 표시, RDSR 생성, DRL 비교, 거부 영상 분석 |
| DICOM 통신 | Storage SCU, MWL SCU, Print SCU 등 (Phase 1); MPPS, Q/R, Commitment (Phase 2) |
| 시스템 관리 | 사용자 관리(RBAC), 시스템 설정, 캘리브레이션 UI |
| 사이버보안 | 인증, 세션 관리, TLS 암호화, Audit Trail, SBOM, CVD, SW 무결성 |
| 규제 준수 문서 | IEC 62304 산출물, IEC 62366 Usability, ISO 14971 연계, STRIDE 위협 모델링 |
...
| Phase | 버전 | 목표 기간 | 핵심 내용 | MR 범위 |
|-------|------|-----------|-----------|---------| 
| **Phase 1** | v1.0 | 출시 후 12개월 | Tier 1 + Tier 2: 핵심 촬영 워크플로우, DICOM 필수 서비스(C-STORE/MWL/Print), 기본 영상 처리, 선량 관리, RBAC, 사이버보안, CD Burning | 30개 MR (Tier 1 × 13 + Tier 2 × 17) |
| **Phase 2** | v2.0 | 이후 12개월 | Tier 3: AI Smart Positioning, MPPS, Storage Commitment, Q/R, RDSR, Image Stitching, Analytics Dashboard, 클라우드 연동 | 25개 MR (Tier 3) |
| **Phase 3+** | v3.0+ | 이후 | Tier 4: AI Noise Cancellation, Cloud SaaS, RESTful API, 레트로핏 | 13개 MR (Tier 4) |
...
| ID | User Story | Tier | Phase |
|----|-----------|------|-------|
| US-001 | As a 방사선사, I want to select a patient from the DICOM Worklist in one tap, so that I can start the exam without manual data entry. | Tier 2 | 1 |
| US-002 | As a 방사선사, I want the system to automatically apply the correct imaging protocol based on the selected body part, so that I minimize protocol selection errors. | Tier 2 | 1 |
| US-003 | As a 방사선사, I want to see the acquired image within 1 second of exposure, so that I can assess image quality and decide on retakes immediately. | Tier 2 | 1 |
| US-004 | As a 방사선사, I want a one-touch emergency registration mode, so that I can quickly start imaging for trauma patients without entering full demographic data. | Tier 2 | 1 |
| US-005 | As a 방사선사, I want the system to display real-time detector status (Ready/Busy/Error), so that I can identify equipment issues before exposure. | Tier 2 | 1 |
| US-006 | As a 방사선사, I want to adjust Window/Level, Zoom, and Pan with touch gestures, so that I can review images naturally and efficiently. | Tier 2 | 1 |
| US-007 | As a 방사선사, I want the system to warn me when the exposure dose exceeds the DRL, so that I can take corrective action to protect the patient. | Tier 3 | 2 |
| US-008 | As a 방사선사, I want to suspend and resume an exam session, so that I can handle interruptions (emergency calls, equipment swaps) without losing data. | Tier 2 | 1 |
| US-009 | As a 방사선사, I want automatic image stitching for full spine/leg-length studies (up to 4 images), so that I can produce panoramic images without manual alignment. | Tier 3 | 2 |
| US-010 | As a 방사선사, I want the system to automatically log rejected images with rejection reason, so that dose and quality metrics are maintained accurately. | Tier 3 | 2 |

#### 선임 방사선사 / 의학물리사

| ID | User Story | Tier | Phase |
|----|-----------|------|-------|
| US-011 | As a 선임 방사선사, I want to create and edit organ programs (APR) with custom kVp/mAs defaults, so that imaging protocols are standardized across the department. | Tier 2 | 1 |
| US-012 | As a 의학물리사, I want to view daily dose reports and compare against DRLs, so that I can identify outliers and optimize radiation protection. | Tier 3 | 2 |
| US-013 | As a 의학물리사, I want the system to generate RDSR automatically after each exam, so that dose data is transmitted to the dose registry without manual effort. | Tier 3 | 2 |
| US-014 | As a 선임 방사선사, I want to review rejected image statistics by technologist/body part, so that I can identify training needs and reduce retake rates. | Tier 3 | 2 |

#### BMET 엔지니어

| ID | User Story | Tier | Phase |
|----|-----------|------|-------|
| US-015 | As a BMET, I want to perform Gain/Offset calibration through a guided wizard UI, so that detector calibration is accurate and repeatable. | Tier 2 | 1 |
| US-016 | As a BMET, I want to access system logs filtered by severity level, so that I can quickly diagnose issues without sifting through irrelevant entries. | Tier 2 | 1 |
| US-017 | As a BMET, I want to configure DICOM network settings (AE Title, IP, port) through a GUI, so that PACS/RIS integration is straightforward. | Tier 2 | 1 |
| US-018 | As a BMET, I want the software to support remote diagnostic access, so that issues can be resolved without an on-site visit when possible. | Tier 3 | 2 |
...
>
> **테이블 형식**: ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적
>
...
| ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|-------|---------|--------|-----------|------|-------|-----------|-----------|-----------| -----------|---------|
| PR-WF-010 | MR-004 | APR 프로토콜 관리 | 신체 부위별 표준 촬영 프로토콜(kVp, mAs, Focus, Grid, AEC chamber) 50개 이상 사전 탑재; 저장·적용 | Tier 2 | 1 | 프로토콜 선택 후 모든 파라미터가 1초 내 Generator/Detector에 자동 적용; 50개 이상 기본 프로토콜 SQLite에 시딩 확인 | SWR-WF-010, SWR-WF-011, SWR-WF-012 | T, D | HAZ-RAD | MR-004 |
| PR-WF-011 | MR-003 | 촬영 순서 설정·변경 | 다중 Projection(뷰)의 촬영 순서를 드래그&드롭으로 재정렬 | Tier 2 | 1 | 순서 변경 후 즉시 시각적 반영; DICOM에 올바른 순서로 기록 | SWR-WF-013, SWR-WF-014 | T, I | HAZ-DATA | MR-003 |
| PR-WF-012 | MR-004, MR-031 | 촬영 조건 설정 | kVp, mAs(또는 mA + 시간), AEC 모드(chamber 선택), 포커스(large/small), SID 입력; Generator 통신으로 kVp/mAs 표시 | Tier 2 | 1 | 범위 초과 값 입력 시 즉시 경고; Generator로 설정값 전송 확인 응답 수신; AEC 파라미터 모니터링 동작 확인 | SWR-WF-015, SWR-WF-016, SWR-WF-017 | T, A | HAZ-RAD | MR-004, MR-031 |
| PR-WF-013 | MR-002 | Generator 통신 및 제어 | Serial/Ethernet 기반 Generator 프로토콜 구현; 촬영 준비(Ready) 신호 수신, 노출 실행, 노출 완료(Exposure End) 이벤트 처리 | Tier 2 | 1 | 노출 시작 명령 후 ≤200ms 내 Generator 응답 수신; 타임아웃 시 에러 코드 표시 | SWR-WF-018, SWR-WF-019, SWR-WF-020 | T, A | HAZ-RAD, HAZ-SW | MR-002 |
| PR-WF-014 | MR-010 | Detector 상태 모니터링 | 최대 4개 Detector 연결 상태(Connected/Disconnected), 준비 상태(Ready/Busy/Error), 배터리(무선), 온도 표시; 클릭으로 활성 디텍터 전환; 5초 주기 폴링 | Tier 2 | 1 | 상태 변경 시 ≤1초 내 GUI 업데이트; Critical 오류 시 경고음 + 시각적 알림; 상태 아이콘 색상 코딩 정확성 확인 | SWR-WF-021, SWR-WF-022 | T, D | HAZ-RAD, HAZ-SW | MR-010 |
| PR-WF-015 | MR-002, MR-023 | 촬영 실행 및 영상 수신 | Exposure 버튼(소프트웨어) 또는 외부 핸드스위치 입력 처리; 자사 FPD SDK 직접 통신으로 Raw 영상 수신; fo-dicom 5.x 기반 DICOM 인스턴스 생성 | Tier 2 | 1 | 영상 수신 후 처리된 영상 GUI 표시까지 ≤1초; 영상 수신 실패 시 재시도 옵션 제공; Raw→PACS 전송 ≤30초 이내 | SWR-WF-023, SWR-WF-024, SWR-WF-025 | T, A | HAZ-RAD, HAZ-SW | MR-002, MR-023 |
| PR-WF-016 | MR-003 | Emergency/Trauma 워크플로우 | 별도 Trauma 템플릿(다중 부위 연속 촬영)을 원탭으로 실행; 촬영 중 부위 추가 가능 | Tier 2 | 1 | Trauma 모드 진입 ≤2 터치; 각 촬영 결과 즉시 표시 후 다음 촬영으로 자동 진행 | SWR-WF-026, SWR-WF-027 | T, D | HAZ-RAD | MR-003 |
...
| ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|-------|---------|--------|-----------|------|-------|-----------|-----------|-----------| -----------|---------|
| PR-IP-020 | MR-011, MR-002 | 실시간 영상 표시 | 촬영 완료 후 Full Resolution 처리 영상을 ≤1초 내 표시; 외부 영상처리 SDK 비동기 호출 | Tier 2 | 1 | 5MP 영상 기준 표시 지연 ≤1000ms; Frame drop 없이 렌더링; SDK 호출 오류 시 원본 표시 | SWR-IP-020, SWR-IP-021 | T, A | HAZ-RAD, HAZ-SW | MR-011, MR-002 |
| PR-IP-021 | MR-012 | Window/Level 조정 | 마우스 드래그, 터치 제스처, 숫자 직접 입력으로 W/L 조정; Flip(수평/수직), Invert, Rotate 포함 | Tier 2 | 1 | 조정 시 ≤100ms 내 영상 갱신; 사전 설정 프리셋(Bone, Chest, Soft Tissue) 제공 | SWR-IP-022, SWR-IP-023 | T, D | — | MR-012 |
| PR-IP-022 | MR-012 | Zoom | 마우스 휠, 핀치-투-줌 제스처, 버튼으로 50%~2000% 확대/축소 | Tier 2 | 1 | 확대 시 보간(bicubic) 적용; Fit-to-Screen, 1:1 단축키 제공 | SWR-IP-024, SWR-IP-025 | T, D | — | MR-012 |
| PR-IP-023 | MR-012 | Pan | 마우스 드래그, 터치 슬라이드로 영상 이동 | Tier 2 | 1 | 영상 경계 밖으로 이동 방지 옵션; 부드러운 이동(jitter 없음) | SWR-IP-026 | T, D | — | MR-012 |
| PR-IP-024 | MR-012 | Rotation | 0°/90°/180°/270° 및 자유 회전(free rotation ±360°) | Tier 2 | 1 | 회전 후 영상 품질 유지; Flip(수평/수직) 포함 | SWR-IP-027, SWR-IP-028 | T, D | — | MR-012 |
| PR-IP-025 | MR-015 | Image Stitching | 동일 검사의 최대 4장 영상을 자동으로 이어붙여 파노라마 영상 생성 (전척추, 하지 전장) | Tier 3 | 2 | 정합 오차 ≤1mm (기준 팬텀); 스티칭 완료 시간 ≤10초 | SWR-IP-029, SWR-IP-030, SWR-IP-031 | T, A | — | MR-015 |
| PR-IP-026 | MR-012 | 거리 측정 | 두 점 간 거리를 캘리브레이션된 픽셀 크기 기준으로 측정 (mm 단위) | Tier 2 | 1 | 측정 결과 ±1% 이내 정확도 (DICOM Pixel Spacing 기준) | SWR-IP-032, SWR-IP-033 | T, A | HAZ-SW | MR-012 |
| PR-IP-027 | MR-012 | 각도 측정 | 두 선분 사이 각도 측정 | Tier 2 | 1 | 0.1° 단위 표시 | SWR-IP-034 | T, A | — | MR-012 |
| PR-IP-028 | MR-012 | 면적 측정 | ROI(관심 영역) 내 면적 및 평균 픽셀값 측정 | Tier 2 | 1 | 원형/다각형 ROI 지원; 면적 cm² 단위 표시 | SWR-IP-035, SWR-IP-036 | T, A | — | MR-012 |
| PR-IP-029 | MR-012 | Annotation | 텍스트 레이블, 화살표, 원/사각형 도형 오버레이; 영상과 함께 DICOM에 저장 | Tier 2 | 1 | Annotation 포함 DICOM Presentation State(PR) 저장; 편집/삭제 가능 | SWR-IP-037, SWR-IP-038 | T, I | HAZ-DATA | MR-012 |
| PR-IP-030 | MR-011 | Gain/Offset 보정 | Flat Field Correction: 각 픽셀에 획득된 Gain/Offset 맵 적용; 외부 SDK 파라미터 연동 | Tier 2 | 1 | 보정 후 VNUE(Variation in Non-Uniformity) ≤1%; 보정 미적용 Raw 영상과 비교 표시 가능 | SWR-IP-039, SWR-IP-040 | T, A | HAZ-RAD, HAZ-SW | MR-011 |
| PR-IP-031 | MR-013 | 노이즈 감소 (Noise Reduction) | 외부 SDK Noise Reduction 파라미터 호출; 주파수 기반 노이즈 제거 | Tier 2 | 1 | SNR 향상 ≥3dB (vs. 미적용); 처리 시간 ≤500ms | SWR-IP-041, SWR-IP-042 | T, A | HAZ-RAD | MR-013 |
| PR-IP-032 | MR-013 | Edge Enhancement | 외부 SDK Edge Enhancement 파라미터 호출; 소형 구조물, 골 세부, 카테터 모드 | Tier 2 | 1 | 각 모드별 강도 5단계 조절; 적용/미적용 토글 가능 | SWR-IP-043, SWR-IP-044 | T, D | — | MR-013 |
| PR-IP-033 | MR-017 | Scatter Correction | 그리드(Grid) 없이 획득한 영상에서 산란선 제거 (SW 기반) | Tier 3 | 2 | 산란선 보정 후 Contrast Ratio 향상 ≥10%; 처리 시간 ≤1초 | SWR-IP-045, SWR-IP-046 | T, A | HAZ-RAD | MR-017 |
| PR-IP-034 | MR-011 | Auto-trimming/Cropping | 조사야(照射野) 바깥 영역 자동 인식 및 트리밍; 외부 SDK 호출 | Tier 2 | 1 | 조사야 경계 인식 정확도 ≥95%; 수동 조정 가능 | SWR-IP-047, SWR-IP-048 | T, A | — | MR-011 |
...
| ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|-------|---------|--------|-----------|------|-------|-----------|-----------|-----------| -----------|---------|
| PR-DM-040 | MR-031 | DAP 표시·기록 | DAP 미터에서 수신한 DAP(Dose Area Product) 값을 매 노출마다 화면 표시 및 DB 기록 | Tier 2 | 1 | 노출 후 ≤2초 내 DAP 값 표시; μGy·cm² 단위 | SWR-DM-040, SWR-DM-041 | T, A | HAZ-RAD | MR-031 |
| PR-DM-041 | MR-031 | ESD 계산 | kVp, mAs, Focus-Skin Distance(FSD), 부위 두께 기반 ESD(Entrance Surface Dose) 추정 계산 | Tier 2 | 1 | 계산 방법 및 불확도(±20%) 사용자에게 고지 | SWR-DM-042, SWR-DM-043 | T, A | HAZ-RAD | MR-031 |
| PR-DM-042 | MR-027 | RDSR 생성 | 검사 완료 후 IEC 60601-2-54 및 DICOM TID-10011 기반 RDSR(Radiation Dose Structured Report) 자동 생성 | Tier 3 | 2 | DICOM Conformance에 정의된 IOD 완전성 100%; Dose Registry(PACS/RIS)로 자동 전송 | SWR-DM-044, SWR-DM-045, SWR-DM-046 | T, I | HAZ-RAD, HAZ-DATA | MR-027 |
| PR-DM-043 | MR-031, MR-028 | Exposure Index 모니터링 | IEC 62494-1 기반 EI(Exposure Index) 및 DI(Deviation Index) 계산 및 표시 | Tier 2 | 1 | DI ±2 이상 시 색상 경고(황색/적색); 각 뷰별 EI 값 저장 | SWR-DM-047, SWR-DM-048 | T, A | HAZ-RAD | MR-031, MR-028 |
...
> **v2.0 변경사항**: MRD v3.0에 따라 Phase 1 범위를 C-STORE SCU + MWL SCU + Print SCU로 조정. MPPS, Storage Commitment, Q/R는 Phase 2(Tier 3)로 이관. fo-dicom 5.x 기반 구현 명시. IHE SWF Basic (RAD-5, RAD-8 트랜잭션) Phase 1 준수.

| ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|-------|---------|--------|-----------|------|-------|-----------|-----------|-----------| -----------|---------|
| PR-DC-050 | MR-019, MR-021 | Storage SCU (C-STORE) | fo-dicom 5.x DicomClient를 사용하여 획득 영상을 PACS/Storage Server로 C-STORE 전송; IHE RAD-8 트랜잭션 구현; Raw 수신부터 PACS 도달까지 ≤30초 | Tier 1 | 1 | 전송 성공률 ≥99.9%; 전송 실패 시 자동 재시도(최대 3회, 지수 백오프) + 로컬 임시 저장; 30초 타임라인 측정 가능 | SWR-DC-050, SWR-DC-051, SWR-DC-052 | T, A | HAZ-DATA | MR-019, MR-021 |
...
sequenceDiagram
    ...
    participant CON as HnVue Console SW
    participant RIS as RIS/HIS
    participant DET as Detector (자사 FPD)
    participant GEN as Generator
    participant PACS as PACS

    RT->>CON: 검사 시작 요청
    CON->>RIS: C-FIND (MWL Query, RAD-5) [PR-DC-051]
    RIS-->>CON: Worklist Items 반환
    CON-->>RT: 검사 목록 표시

    RT->>CON: 환자/검사 선택
    Note over CON: MPPS는 Phase 2 (MR-009)
    ...
    DET-->>CON: Raw Image 전송 (FPD SDK) [PR-WF-015]

    CON->>CON: 영상 처리 파이프라인 (외부 SDK) [PR-IP-030~037]
    CON-->>RT: 처리 영상 표시 (≤1초) [PR-IP-020]

    RT->>CON: 영상 승인 (Accept)
    CON->>PACS: C-STORE (DICOM Image, RAD-8) [PR-DC-050]
    PACS-->>CON: C-STORE 성공 응답 (≤30초)

    CON-->>RT: 검사 완료 표시 (전송 ≤30초)
```
...
| PR-SA-064 | MR-023 | Defect Pixel Map 관리 | 결함 픽셀 자동 감지 및 수동 추가/제거; 결함 픽셀 맵 시각화 | Tier 2 | 1 | 결함 픽셀 수 및 분포 통계 표시 | SWR-SA-070, SWR-SA-071 | T, A | HAZ-RAD | MR-023 |
| PR-SA-065 | MR-035 | Audit Trail | 모든 임상·설정·보안 관련 이벤트(로그인/로그아웃, 설정 변경, 영상 승인/거부, 데이터 전송)를 타임스탬프와 함께 기록; Serilog 기반; SHA-256 해시 체인으로 변조 방지; 365일 보관 | Tier 1 | 1 | IHE ATNA 프로파일 준수; 감사 로그 변조 방지(SHA-256 해시 체인 검증); 365일 이상 보관 확인; Serilog 출력 확인 | SWR-SA-072, SWR-SA-073 | T, I | HAZ-SEC, HAZ-DATA | MR-035 |
| PR-SA-066 | MR-044, MR-048 | 로깅 시스템 및 오류 메시지 | Error, Warning, Info, Debug 4단계 로그 수준; 구조화된 로그 파일 출력; 오류 메시지는 원인 + 해결 방법 명시 | Tier 2 | 1 | 로그 파일 자동 순환(최대 500MB, 30일); Log Level 런타임 변경 가능; 오류 메시지에 기술적 코드만 표시되지 않도록 검증 | SWR-SA-074, SWR-SA-075 | T, I | HAZ-SW | MR-044, MR-048 |
| PR-SA-067 | MR-039 | SW 업데이트 관리 | 서명된 업데이트 패키지 검증 후 설치; 롤백 지원; 업데이트 무결성 검증 (FDA 524B(b)(2) 요구사항) | Tier 1 | 1 | 업데이트 전 DB/설정 자동 백업; 서명 검증 실패 시 설치 차단; 롤백 성공 확인; Code Signing 인증서 EV 등급 확인 | SWR-SA-076, SWR-SA-077 | T, I | HAZ-SEC, HAZ-SW | MR-039 |

### 3.7 사이버보안 (Cybersecurity) — PR-CS

> **v2.0 변경사항**: MRD v3.0 Tier 1 보안 항목 상세 반영. MR-037 CVD + 인시던트 대응, MR-039 SW 무결성 + 업데이트 메커니즘, MR-050 STRIDE 위협 모델링 추가.

| ID | 출처 MR | 요구사항명 | 상세 설명 | Tier | Phase | 수용 기준 | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|-------|---------|--------|-----------|------|-------|-----------|-----------| -----------|---------|---------|
| PR-CS-070 | MR-033 | 로컬 사용자 인증 | 사용자명/비밀번호 기반 로컬 인증; bcrypt 해시 저장; 로그인 실패 5회 시 계정 잠금(30분 또는 관리자 해제); 마지막 로그인 정보 표시 | Tier 1 | 1 | 로그인 실패 5회 잠금 동작 확인; bcrypt 해시 저장 확인; 마지막 로그인 시각 표시 | SWR-CS-070, SWR-CS-071, SWR-CS-072 | T, I | HAZ-SEC | MR-033 |
| PR-CS-071 | MR-038 | Domain 인증 (SSO) | Active Directory / LDAP 기반 도메인 인증 지원 | Tier 3 | 2 | Kerberos/NTLM 지원; SSO 실패 시 로컬 계정으로 폴백 옵션 | SWR-CS-073, SWR-CS-074 | T, D | HAZ-SEC | MR-038 |
| PR-CS-072 | MR-033 | 세션 관리 | 비활성 시간(설정 가능, 기본 15분) 경과 후 자동 로그아웃; 촬영 중 세션 만료 방지 | Tier 1 | 1 | 세션 만료 3분 전 경고; 촬영 진행 중 화면 잠금 모드(Quick PIN)로 전환 | SWR-CS-075, SWR-CS-076, SWR-CS-077 | T, D | HAZ-SEC, HAZ-RAD | MR-033 |
| PR-CS-073 | MR-034 | DICOM TLS 암호화 | 모든 DICOM 네트워크 통신에 TLS 1.2 이상 적용; SQLCipher AES-256으로 로컬 DB의 PHI 필드 암호화 | Tier 1 | 1 | TLS 핸드셰이크 성공; 암호화 비활성화 시 관리자 확인 필요; SQLCipher AES-256 DB 암호화 검증 | SWR-CS-078, SWR-CS-079 | T, I | HAZ-SEC | MR-034 |
| PR-CS-074 | MR-034 | PHI 보호 | 로컬 DB의 PHI 필드 SQLCipher AES-256 암호화; 스크린샷/화면 녹화 방지; 복호화는 애플리케이션 내부에서만 수행 | Tier 1 | 1 | DB 암호화 키 분리 저장; 스크린샷 방지 동작 확인 | SWR-CS-080, SWR-CS-081 | T, I | HAZ-SEC | MR-034 |
| PR-CS-075 | MR-036 | SBOM 관리 | 모든 Third-party 소프트웨어 컴포넌트 목록(SBOM) 생성 및 취약점 모니터링; CycloneDX for .NET 형식; NVD 매칭 | Tier 1 | 1 | CycloneDX 형식 SBOM 생성 확인; CVE 발견 시 30일 내 패치 계획 수립; NVD 매칭 동작 확인 | SWR-CS-082, SWR-CS-083 | T, I | HAZ-SEC | MR-036 |
| PR-CS-076 | MR-039 | Secure Boot / SW 무결성 + 업데이트 메커니즘 | 애플리케이션 실행 전 코드 서명 검증; 변조 감지 시 실행 차단; 서명된 업데이트 패키지; 롤백 지원 (FDA 524B(b)(2) 요구사항) | Tier 1 | 1 | 코드 서명 인증서 EV(Extended Validation) 등급 사용; 검증 실패 로그 기록; 서명 없는 업데이트 차단 확인 | SWR-CS-084, SWR-CS-085 | T, I | HAZ-SEC, HAZ-SW | MR-039 |
| PR-CS-077 | MR-037 | CVD + 보안 인시던트 대응 | 취약점 공개 정책(CVD: Coordinated Vulnerability Disclosure) 프로세스 수립; security@ 전용 채널 운영; 보안 인시던트 대응/복구 계획; SLA 정의; IEC 81001-5-1 Clause 8 요구사항 충족 | Tier 1 | 1 | CVD 정책 문서 작성 완료; security@ 채널 운영 확인; 인시던트 대응 절차 문서화; SLA 정의(Critical: 30일 패치) | SWR-CS-086, SWR-CS-087 | I, A | HAZ-SEC | MR-037 |
...
>
> **테이블 형식**: ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적

### 4.1 성능 (Performance) — PR-NF-PF

| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-PF-001 | MR-011, MR-002 | 성능 | 영상 표시 지연 | ≤1,000ms (Full Resolution, 5MP) | Tier 2 | 1 | SWR-NF-PF-001 | T, A | HAZ-RAD | MR-011, MR-002 |
| PR-NF-PF-002 | MR-001 | 성능 | 환자 검색 응답 | ≤500ms (10,000건 DB 기준) | Tier 2 | 1 | SWR-NF-PF-002 | T, A | — | MR-001 |
| PR-NF-PF-003 | MR-002, MR-019 | 성능 | PACS 전송 속도 (전체 파이프라인) | FPD Raw 수신 완료부터 PACS C-STORE Response까지 ≤30초 (100Mbps LAN 기준) | Tier 2 | 1 | SWR-NF-PF-003 | T, A | HAZ-DATA | MR-002, MR-019 |
| PR-NF-PF-004 | MR-003 | 성능 | GUI 응답 시간 | ≤200ms (버튼/터치 입력 → 피드백) | Tier 2 | 1 | SWR-NF-PF-004 | T, A | — | MR-003 |
| PR-NF-PF-005 | MR-044 | 성능 | 시스템 부팅 시간 | ≤60초 (BIOS POST 이후 → 로그인 화면) | Tier 2 | 1 | SWR-NF-PF-005 | T | — | MR-044 |
| PR-NF-PF-006 | MR-003 | 성능 | 동시 처리 | 최소 3개 Study 병렬 관리 | Tier 2 | 1 | SWR-NF-PF-006 | T, A | HAZ-SW | MR-003 |
| PR-NF-PF-007 | MR-001, MR-025 | 성능 | MWL 새로 고침 | ≤3초 내 Worklist 업데이트 | Tier 2 | 1 | SWR-NF-PF-007 | T, A | — | MR-001, MR-025 |
| PR-NF-PF-008 | MR-015 | 성능 | Image Stitching | ≤10초 (4장 5MP 기준) | Tier 3 | 2 | SWR-NF-PF-008 | T, A | — | MR-015 |
...
| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-RL-010 | MR-002 | 신뢰성 | 시스템 가용성 | ≥99.5% (운영 시간 기준, 계획된 유지보수 제외) | Tier 2 | 1 | SWR-NF-RL-010 | T, A | HAZ-SW | MR-002 |
| PR-NF-RL-011 | MR-002 | 신뢰성 | MTBF | ≥10,000시간 (SW 장애 기준) | Tier 2 | 1 | SWR-NF-RL-011 | A | HAZ-SW | MR-002 |
| PR-NF-RL-012 | MR-002, MR-019 | 신뢰성 | 데이터 무결성 | 촬영 데이터 손실 허용 0건 (전원 차단 시 Write-ahead logging 적용) | Tier 2 | 1 | SWR-NF-RL-012 | T, A | HAZ-DATA | MR-002, MR-019 |
| PR-NF-RL-013 | MR-002 | 신뢰성 | 자동 복구 | SW 크래시 시 ≤30초 내 자동 재시작; 마지막 안정 상태로 복귀 | Tier 2 | 1 | SWR-NF-RL-013 | T, D | HAZ-SW | MR-002 |
| PR-NF-RL-014 | MR-019 | 신뢰성 | Graceful Degradation | 네트워크 연결 실패 시 로컬 저장 후 재연결 시 자동 전송 | Tier 2 | 1 | SWR-NF-RL-014 | T, D | HAZ-DATA | MR-019 |
| PR-NF-RL-015 | MR-002 | 신뢰성 | 장기 운영 안정성 | 72시간 연속 운영 후 메모리 누수 없음 (Heap 증가 ≤10MB/24h) | Tier 2 | 1 | SWR-NF-RL-015 | T, A | HAZ-SW | MR-002 |

### 4.3 사용성 (Usability) — PR-NF-UX

| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-UX-020 | MR-043 | 사용성 | 교육 시간 | 기본 운영 ≤4시간 (Philips 교육 시간 33% 절감 벤치마크 기준) | Tier 3 | 2 | SWR-NF-UX-020 | D, A | — | MR-043 |
| PR-NF-UX-021 | MR-003 | 사용성 | 표준 검사 클릭 수 | Worklist 선택 → 촬영 완료 ≤5회 터치/클릭 (환자선택→프로토콜→촬영→확인→전송) | Tier 2 | 1 | SWR-NF-UX-021 | T, D | HAZ-RAD | MR-003 |
| PR-NF-UX-022 | MR-041 | 사용성 | 터치스크린 최적화 | 최소 터치 타겟 크기 44×44px (Apple HIG / WCAG 2.1 기준) | Tier 3 | 2 | SWR-NF-UX-022 | I | — | MR-041 |
| PR-NF-UX-023 | MR-045 | 사용성 | 다국어 지원 | 한국어, 영어 (Phase 1; .resx 기반 로컬라이제이션); 추가 언어 Phase 2 | Tier 2 | 1 | SWR-NF-UX-023 | T, I | — | MR-045 |
...
| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-CP-030 | MR-044 | 호환성 | 운영체제 | Windows 10 LTSC 2021 / Windows 11 (64-bit) | Tier 2 | 1 | SWR-NF-CP-030 | T, I | HAZ-SW | MR-044 |
| PR-NF-CP-031 | MR-023 | 호환성 | 자사 FPD 통합 | Phase 1: 자사 FPD SDK 직접 통신 (전용); Phase 2: 멀티벤더 5개+ | Tier 2 | 1 | SWR-NF-CP-031 | T, D | HAZ-RAD | MR-023 |
| PR-NF-CP-032 | MR-002 | 호환성 | Multi-vendor Generator | Generator 제조사 ≥3개 벤더 지원 via Serial/Ethernet 프로토콜 | Tier 2 | 1 | SWR-NF-CP-032 | T, D | HAZ-RAD | MR-002 |
| PR-NF-CP-033 | MR-021 | 호환성 | PACS 호환성 | 주요 3개 벤더 이상 PACS와 호환 검증 | Tier 2 | 1 | SWR-NF-CP-033 | T, D | HAZ-DATA | MR-021 |
| PR-NF-CP-034 | MR-011 | 호환성 | 모니터 해상도 | 3MP(2048×1536) 이상 의료용 모니터 지원; 4K(3840×2160) 지원 | Tier 2 | 1 | SWR-NF-CP-034 | T, I | — | MR-011 |
| PR-NF-CP-035 | MR-019, MR-020 | 호환성 | DICOM 표준 | DICOM 2023a 이상 준수; IHE SWF Basic (RAD-5, RAD-8) Phase 1 지원 | Tier 1 | 1 | SWR-NF-CP-035 | T, I | HAZ-DATA | MR-019, MR-020 |

### 4.5 보안 (Security) — PR-NF-SC

| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-SC-040 | MR-037, MR-039 | 보안 | 규제 준수 | FDA Section 524B FD&C Act, HIPAA Security Rule | Tier 1 | 1 | SWR-NF-SC-040 | I, A | HAZ-SEC | MR-037, MR-039 |
| PR-NF-SC-041 | MR-034 | 보안 | 암호화 전송 | TLS 1.2 이상 (TLS 1.0/1.1 비활성화) | Tier 1 | 1 | SWR-NF-SC-041 | T, I | HAZ-SEC | MR-034 |
| PR-NF-SC-042 | MR-033 | 보안 | 비밀번호 정책 | 최소 8자, 대소문자+숫자+특수문자 조합; 90일 만료; 이전 5개 비밀번호 재사용 금지 | Tier 1 | 1 | SWR-NF-SC-042 | T, I | HAZ-SEC | MR-033 |
| PR-NF-SC-043 | MR-033 | 보안 | 접근 제어 | RBAC; 최소 권한 원칙(Principle of Least Privilege) | Tier 1 | 1 | SWR-NF-SC-043 | T, I | HAZ-SEC | MR-033 |
| PR-NF-SC-044 | MR-036 | 보안 | 취약점 관리 | 배포 전 SAST/DAST 도구 통과; Critical/High CVE 0건; NVD 매칭 | Tier 1 | 1 | SWR-NF-SC-044 | T, A | HAZ-SEC | MR-036 |
| PR-NF-SC-045 | MR-035 | 보안 | Audit Log 보관 | ≥365일 보관; IHE ATNA 프로파일 준수; SHA-256 해시 체인 무결성 | Tier 1 | 1 | SWR-NF-SC-045 | T, I | HAZ-SEC | MR-035 |

### 4.6 유지보수성 (Maintainability) — PR-NF-MT

| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-MT-050 | MR-044 | 유지보수성 | 모듈식 아키텍처 | 각 서브시스템(환자관리, 촬영, 영상처리, DICOM)이 독립 모듈로 분리 | Tier 2 | 1 | SWR-NF-MT-050 | I, A | HAZ-SW | MR-044 |
| PR-NF-MT-051 | MR-050 | 유지보수성 | 코드 커버리지 | ≥80% 라인 커버리지 (IEC 62304 Class B) | Tier 1 | 1 | SWR-NF-MT-051 | T, A | HAZ-SW | MR-050 |
| PR-NF-MT-052 | MR-052 | 유지보수성 | API 문서화 | 내부 API 100% 문서화 (XML Doc Comments + Swagger/OpenAPI) | Tier 1 | 1 | SWR-NF-MT-052 | I | — | MR-052 |
| PR-NF-MT-053 | MR-044 | 유지보수성 | 원격 진단 | VPN 기반 원격 접속을 통한 로그 수집, 설정 변경 지원 | Tier 3 | 2 | SWR-NF-MT-053 | T, D | HAZ-SEC | MR-044 |
| PR-NF-MT-054 | MR-052 | 유지보수성 | 빌드 자동화 | CI/CD 파이프라인 (GitHub Actions + NUnit/Google Test) | Tier 1 | 1 | SWR-NF-MT-054 | I, D | — | MR-052 |

### 4.7 규제 준수 (Regulatory Compliance) — PR-NF-RG

| ID | 출처 MR | 카테고리 | 요구사항 | 기준치 | Tier | Phase | 파생 SWR | 검증 수준 | 위험 참조 | MR 추적 |
|--------|---------|---------|---------|--------|------|-------|----------|-----------|-----------|---------|
| PR-NF-RG-060 | MR-050 | 규제 준수 | IEC 62304 적합성 + STRIDE 위협 모델링 | Class B 전 단계 산출물 완비; STRIDE 기반 위협 모델링 수행 및 문서화 (IEC 81001-5-1 Clause 5.2) | Tier 1 | 1 | SWR-NF-RG-060 | I, A | HAZ-SW | MR-050 |
| PR-NF-RG-061 | MR-051, MR-052 | 규제 준수 | ISO 14971 연계 | 전체 PR/PR-NF에 위험 참조(HAZ-xxx) 매핑; Risk Control Measure 역추적 가능 | Tier 1 | 1 | SWR-NF-RG-061 | I, A | HAZ-RAD, HAZ-SW | MR-051, MR-052 |
| PR-NF-RG-062 | MR-051 | 규제 준수 | IEC 62366 Usability | Summative Usability Test 완료 및 보고서 제출; 안전 관련 Task 완료율 ≥95% | Tier 1 | 1 | SWR-NF-RG-062 | D, A | HAZ-RAD | MR-051 |
...
> **v2.0 변경사항**: MR-023(자사 FPD 통합), MR-010(다중 디텍터 상태 표시) 상세 반영. Phase 1 = 자사 FPD SDK 직접 통신 전용.

| 인터페이스 | 통신 방식 | 프로토콜/표준 | 설명 | MR 추적 |
|-----------|-----------|--------------|------|---------|
| X-Ray Generator | Ethernet / RS-232 / RS-485 | 벤더별 독점 프로토콜 (HAL 추상화 레이어) | 촬영 조건 설정, 노출 명령, 노출 완료 이벤트, 에러 상태 수신; kVp/mAs 표시 | MR-002 |
| 자사 Flat Panel Detector (FPD) | GigE Vision / 자사 SDK | 자사 FPD SDK 직접 통신 (Phase 1 전용) | 트리거 신호, Raw 영상 데이터 수신, 배터리/온도 상태; 최대 4개 디텍터 상태 표시 | MR-023, MR-010 |
| AEC (Auto Exposure Control) | Generator 내장 또는 별도 | 아날로그/디지털 인터페이스 | AEC 챔버 선택, 감도 설정, kVp/mAs 모니터링 | MR-031 |
| DAP Meter | RS-232 / USB | DICOM TID 10011 호환 | 선량 데이터(μGy·cm²) 수신 | MR-031 |
| Hand Switch | GPIO / USB HID | HID 표준 | 외부 촬영 버튼 이벤트 | MR-002 |
| Barcode Scanner | USB HID | HID 표준 | 환자 ID 바코드/QR 스캔 | MR-001 |
| CD/DVD Writer | USB / SATA | 표준 광학 드라이브 인터페이스 | DICOMDIR + 뷰어 + ISO 이미지 굽기 | MR-072 |

### 5.2 소프트웨어 인터페이스 (Software Interfaces)

| 시스템 | 프로토콜 | 표준 | 데이터 방향 | MR 추적 |
|--------|---------|------|------------|---------|
| PACS | DICOM 3.0 | Storage SCU (C-STORE), Print SCU (Phase 1); Q/R SCU, Commitment SCU (Phase 2) | 영상 전송, 이전 영상 조회 | MR-019, MR-021 |
| RIS | DICOM 3.0 | MWL SCU (RAD-5, Phase 1); MPPS SCU (RAD-6/7, Phase 2) | 검사 목록 수신, 상태 보고 | MR-001, MR-025 |
| HIS | HL7 v2.x / FHIR R4 | ADT 메시지 | 환자 인구통계 수신 | MR-001 |
| Dose Registry | DICOM 3.0 | C-STORE (RDSR) | 선량 보고서 전송 (Phase 2) | MR-027 |
| Print Server | DICOM 3.0 | Basic Grayscale Print SCU | 필름 인쇄 | MR-024 |
| AD/LDAP | Kerberos / LDAP v3 | RFC 4511 | 사용자 인증 (Phase 2) | MR-038 |
| OS 방화벽/백신 | Windows API | Windows Security Center API | 보안 상태 모니터링 | MR-037 |
| 외부 영상처리 SDK | C++ DLL / .NET API | 벤더 API | 영상처리 파이프라인 (Phase 1) | MR-011, MR-013 |
...
flowchart TB
    ...
    CON -- "C-STORE (RAD-8)\nPrint SCU (Phase 1)" --> PACS
    RIS -- "MWL 검사 목록\nC-FIND (RAD-5)" --> CON
    CON -- "MPPS 상태 보고 (Phase 2)" --> RIS
    CON -- "RDSR 전송 (Phase 2)" --> DOSE
    CON -- "필름 인쇄 요청" --> PRINT
...
| 영상 처리 엔진 | **외부 SDK (Phase 1)** + C++ (Phase 2 내재화) | Phase 1 신속 구현; Phase 2 자체 엔진 내재화 |
| GUI-영상처리 브릿지 | **C++/CLI 또는 P/Invoke** | C# WPF ↔ C++ 처리 엔진 연동 |
| 로컬 데이터베이스 | **SQLite 3.x + SQLCipher** | 경량, 임베디드, ACID 준수; SQLCipher로 PHI AES-256 암호화 |
| DICOM 영상 저장 | **DICOM 파일 시스템** (.dcm) + SQLite 메타데이터 | 표준 준수, PACS 호환 |
| DICOM 라이브러리 | **fo-dicom 5.x (C#)** | C-STORE SCU, MWL SCU, Print SCU 구현; DicomClient 기반 |
| 로깅 | **Serilog** | 구조화 로깅, SHA-256 해시 체인 Audit Trail |
| SBOM | **CycloneDX for .NET** | FDA 524B SBOM 법적 요건 |
...
| 로컬라이제이션 | **.resx 기반** | 한/영 2개 언어 Phase 1 지원 |
...
classDiagram
    ...
    class IDicomService {
        ...
        +BurnToDicomCD(images, path) bool
    }
...
- **HAL (Hardware Abstraction Layer)**: Generator/Detector 벤더별 드라이버를 인터페이스 뒤에 캡슐화하여 멀티벤더 지원 (Phase 2)
- **Plugin Architecture**: Detector SDK 플러그인으로 새 벤더 추가 시 재컴파일 불필요 (Phase 2)
- **설정 외재화**: 모든 하드웨어 파라미터를 XML 설정 파일로 관리 (코드 변경 없이 기기 적응)
- **로컬 우선 (Offline First)**: 네트워크 단절 시 로컬 DB에 저장 후 자동 동기화
- **Security by Design**: SQLCipher AES-256, bcrypt, SHA-256 해시 체인, Code Signing을 아키텍처 초기부터 적용
- **.resx 기반 로컬라이제이션**: 한/영 2개 언어 지원 (Phase 1); 언어 리소스 분리로 추가 언어 확장 용이
...
| 표준/규제 | 해당 사항 | 요구 활동 | MR 추적 |
|-----------|-----------|-----------|---------|
| **IEC 62304:2006+A1:2015** | Class B SW | SW 개발 계획, 요구사항 분석, 아키텍처/상세 설계, 단위 검증, 통합 테스트, 시스템 테스트, 릴리스 | MR-050 |
| **IEC 62366-1:2015** | Usability Engineering | Use Specification, 잠재적 Use Error 분석, Formative/Summative 평가 | MR-051 |
| **ISO 14971:2019** | Risk Management | 위험 관리 계획, 위험 식별/분석/평가, 위험 완화, 잔류 위험 허용성 평가 | MR-051, MR-052 |
| **FDA 21 CFR Part 820** | Quality System Regulation | 설계 제어(Design Control), 문서 관리, 시정 및 예방 조치(CAPA) | MR-052 |
| **FDA Section 524B** | Cybersecurity | Premarket Cybersecurity 요구사항, SBOM 제출, CVD 정책, 인시던트 대응 | MR-036, MR-037, MR-039 |
| **IEC 81001-5-1** | Health Software Security | STRIDE 위협 모델링 (Clause 5.2), CVD + 인시던트 대응 (Clause 8) | MR-050, MR-037 |
| **HIPAA Security Rule** | PHI 보호 | 기술적 보호 조치(암호화, 접근 제어, Audit Trail) | MR-034, MR-035 |
| **IHE Technical Framework** | Interoperability | SWF Basic Profile (RAD-5, RAD-8), ATNA Profile | MR-019, MR-020 |
| **ISO 13485:2016** | 품질 경영 시스템 | 의료기기 설계/개발 품질 요구사항 (Clause 7.3) | MR-052 |
...
| SBOM 취약점 스캔 | Critical/High CVE 0건 | CycloneDX + NVD |
...
### 8.1 Phase별 핵심 기능 (MRD v3.0 4-Tier 기반 재정리)

| Phase | 버전 | 기간 | 핵심 기능 | MR 범위 | 규제 마일스톤 |
|-------|------|------|-----------|---------|--------------|
| **Phase 1** | v1.0 | 12개월 | **Tier 1 (13개)**: DICOM 3.0 C-STORE/MWL/Print, IHE SWF Basic(RAD-5/8), RBAC(bcrypt/5회잠금), PHI AES-256(SQLCipher), 감사로그(Serilog/SHA-256/365일), SBOM(CycloneDX), CVD+인시던트대응, SW무결성+업데이트메커니즘, IEC62304+STRIDE위협모델링, IEC62366, ISO13485/21CFR820, 규제승인, DICOM Conformance Statement<br>**Tier 2 (17개)**: MWL 자동조회(10초폴링), PACS전송≤30초, 5클릭워크플로우, 50개+프로토콜, 다중디텍터상태, CD/DVD Burning, 자동영상최적화(외부SDK), W/L/Zoom/Pan, 후처리Preset, PACS3개+상호운용성, 자사FPD통합, DICOM Print, RIS MWL, AEC모니터링, 시스템상태표시, 한/영언어(.resx), 오류메시지안내 | Tier 1+2 = 30개 MR | IEC 62304 SW 개발 완료, MFDS 인허가 신청 |
| **Phase 2** | v2.0 | +12개월 | **Tier 3 (25개)**: MPPS, Storage Commitment, Q/R SCU, RDSR, DRL 비교, 선량통계, Reject Analysis, Image Stitching, Scatter Correction, SNR/MTF 측정, SSO/AD, 터치스크린, 화면커스터마이징, AI Smart Positioning 등 | Tier 3 = 25개 MR | FDA 510(k) / CE MDR 준비 |
| **Phase 3+** | v3.0+ | +이후 | **Tier 4 (13개)**: AI 노이즈캔슬레이션, Cloud SaaS, RESTful API, 레트로핏, AI 플러그인 아키텍처 등 | Tier 4 = 13개 MR | 업데이트 신고 |
...
gantt
    ...
    보안/RBAC/SBOM 모듈 개발         :dev6, 2026-04, 2026-07
    CD Burning 기능 개발             :dev7, 2026-06, 2026-08
    ...
    SSO/AD 인증 개발                 :dev11, 2027-05, 2027-08
...
|---------|-----------|-----------|--------|--------|-----------| 
| TR-001 | 자사 FPD SDK 통합 문제 (Phase 1 전용) | 낮음 | 높음 | **High** | 조기 PoC 수행; FPD SDK 문서 선행 검토; HAL 추상화로 향후 멀티벤더 대비 |
| TR-002 | 영상 처리 파이프라인 성능 (≤1초) 미달 | 중간 | 높음 | **High** | 외부 SDK 비동기 호출(Task.Run); 조기 성능 프로파일링; Phase 2에서 C++ 엔진 내재화 |
| TR-003 | DICOM 호환성 문제 (PACS 벤더별 방언) | 높음 | 중간 | **High** | 3개 이상 PACS와 연동 테스트 환경 구축; fo-dicom 5.x 기반 검증 |
| TR-004 | PACS 전송 ≤30초 성능 미달 | 중간 | 높음 | **High** | 비동기 파이프라인 설계; 100Mbps 테스트 환경 조기 구축 |
| TR-005 | 사이버보안 취약점 (PHI 노출) | 낮음 | 최고 | **Critical** | 개발 초기부터 SAST/DAST 적용; CycloneDX SBOM NVD 매칭; 외부 침투 테스트 (연 1회) |
| TR-006 | STRIDE 위협 모델링 결과 아키텍처 재설계 필요 | 낮음 | 중간 | **Medium** | Phase 1 초기(M2)에 STRIDE 수행; 아키텍처 설계와 동시에 진행 |

### 9.2 일정 위험 (Schedule Risks)

| 위험 ID | 위험 설명 | 발생 가능성 | 영향도 | 위험도 | 완화 방안 |
|---------|-----------|-----------|--------|--------|-----------| 
| SR-001 | 핵심 개발 인력 이탈 (SW 2명) | 중간 | 높음 | **High** | 지식 문서화 의무화; 기능별 Cross-training |
| SR-002 | 요구사항 변경 (Scope Creep) | 높음 | 중간 | **High** | Change Control Board 운영; Phase 2 기능은 Parking Lot 관리 |
| SR-003 | 하드웨어 연동 지연 (자사 FPD/Generator 수급) | 낮음 | 중간 | **Medium** | 소프트웨어 시뮬레이터 병행 개발 |
| SR-004 | Usability 테스트 반복으로 인한 일정 지연 | 중간 | 중간 | **Medium** | Formative 테스트를 개발 초기(Sprint 4)부터 주기적 수행 |

### 9.3 규제 위험 (Regulatory Risks)

| 위험 ID | 위험 설명 | 발생 가능성 | 영향도 | 위험도 | 완화 방안 |
|---------|-----------|-----------|--------|--------|-----------| 
| RR-001 | FDA 510(k) 심사 지연 | 중간 | 높음 | **High** | 규제 전문가(RA) 조기 투입; 사전 Pre-Sub Meeting 신청 |
| RR-002 | IEC 62304 문서 불완전으로 인증 실패 | 중간 | 높음 | **High** | Notified Body와 조기 인터랙션; Gap Analysis 수행 |
| RR-003 | FDA Cybersecurity 가이던스 변경 (Section 524B) | 낮음 | 높음 | **Medium** | FDA 가이던스 변경 모니터링; 보안 모듈 교체 가능 설계 |
| RR-004 | MFDS 인허가 요건 추가 요구사항 | 중간 | 중간 | **Medium** | MFDS 규제 담당자와 사전 상담 |
| RR-005 | PHI 처리 관련 HIPAA/개인정보보호법 위반 | 낮음 | 최고 | **Critical** | Privacy by Design 원칙 적용; 법무 검토 및 PIA 수행 |
...
본 제품은 IEC 62304 Class B 분류에 따라 아래 V-Model 체계를 따른다.
...
| **Exit Criteria** | 모든 인터페이스 시나리오 Pass; Critical 결함 0건; 자사 FPD 시뮬레이터 전 시나리오 Pass |
...
| **Exit Criteria** | 모든 PR/PR-NF 수용 기준 Pass; Critical/High 미결 결함 0건; PACS 전송 ≤30초 확인 |
...
| PR-DM-042 | RDSR 생성 | 선량 기록 누락으로 선량 추적 불가 | RDSR 자동 생성 필수(Phase 2); 생성 실패 시 재시도 + 알림; 로컬 백업 | Low |
| PR-DC-050 | Storage SCU | PACS 전송 실패로 영상 손실 | 자동 재시도 3회; 로컬 임시 저장; 30초 이내 전송 SLA | Low |
| PR-NF-RL-012 | 데이터 무결성 | 전원 차단 시 촬영 데이터 손실 | Write-ahead logging; 데이터 손실 허용 0건 | Low |
| PR-WF-019 | CD/DVD Burning | 매체 기록 중 오류로 데이터 손상 | 체크섬 검증; 기록 실패 시 명시적 오류 메시지; 재시도 옵션 | Low |

#### 보안 위험 (HAZ-SEC) 관련 요구사항

| 요구사항 ID | 기능명 | 위험 시나리오 | 위험 통제 조치 | 잔류 위험 |
|-----------|--------|------------|--------------|---------| 
| PR-CS-070 | 로컬 사용자 인증 | 무단 접근으로 PHI 노출 | 로그인 실패 5회 잠금; bcrypt 해시; 마지막 로그인 표시 | Low |
| PR-CS-074 | PHI 보호 | DB 탈취 시 환자 정보 노출 | SQLCipher AES-256 암호화; 키 분리 저장; 스크린샷 방지 | Low |
| PR-CS-076 | Secure Boot / SW 무결성 | 악성코드 삽입 후 실행 | EV 코드 서명 검증; 변조 감지 시 실행 차단; 로그 기록 | Low |
| PR-CS-077 | CVD + 보안 인시던트 대응 | 취약점 미공개 또는 인시던트 미대응 | security@ 전용 채널; SLA 정의; 인시던트 대응 절차 문서화 | Low |
...
본 PRD의 모든 요구사항(PR-xxx, PR-NF-xxx)은 FDA 21 CFR 820.30 설계 제어(Design Controls) 체계에서 **Design Input**으로 기능한다.
...
> MRD v3.0 4-Tier 체계 기반, MR → PR → SWR 3단계 추적성 (v2.0 전면 재작성)

### A.1 RTM 구조 및 범례

| 컬럼 | 설명 |
|------|------|
| MR ID | MRD v3.0의 Market Requirement ID (출처) |
| Tier | MRD v3.0 4-Tier 분류 |
...
| MR ID | Tier | PR-NF ID | 파생 SWR | Design Element | Verification Method | Test Case ID | Validation ID | HAZ ID | Status |
|-------|------|----------|---------|----------------|---------------------|--------------|---------------|--------|--------|
| MR-002, MR-003 | Tier 2 | PR-NF-PF-001 | SWR-NF-PF-001 | SAD: ImageProc / 전체 처리 파이프라인 | T, A | ST-PR-NF-PF-001 | VT-CL-001 | HAZ-RAD | Open |
| MR-001 | Tier 2 | PR-NF-PF-002 | SWR-NF-PF-002 | SAD: PatientMgmt / SDS: SearchIndex | T, A | ST-PR-NF-PF-002 | VT-UX-001 | — | Open |
| MR-002 | Tier 2 | PR-NF-PF-003 | SWR-NF-PF-003 | SAD: DicomComm / 네트워크 스택 | T, A | ST-PR-NF-PF-003 | VT-CL-001 | HAZ-DATA | Open |
| MR-003 | Tier 2 | PR-NF-PF-004 | SWR-NF-PF-004 | SAD: UI / 이벤트 핸들러 | T, A | ST-PR-NF-PF-004 | VT-UX-002 | — | Open |
| MR-044 | Tier 2 | PR-NF-PF-005 | SWR-NF-PF-005 | SAD: SystemInit / 부팅 시퀀스 | T | ST-PR-NF-PF-005 | VT-UX-001 | — | Open |
| MR-003 | Tier 2 | PR-NF-PF-006 | SWR-NF-PF-006 | SAD: Acquisition / 멀티스레드 처리 | T, A | ST-PR-NF-PF-006 | VT-CL-002 | HAZ-SW | Open |
| MR-001, MR-025 | Tier 2 | PR-NF-PF-007 | SWR-NF-PF-007 | SAD: DicomComm / MWL 캐시 | T, A | ST-PR-NF-PF-007 | VT-UX-001 | — | Open |
| MR-015 | Tier 3 | PR-NF-PF-008 | SWR-NF-PF-008 | SAD: ImageProc / StitchingEngine | T, A | ST-PR-NF-PF-008 | VT-CL-002 | — | Open |
| MR-003 | Tier 2 | PR-NF-RL-010 | SWR-NF-RL-010 | SAD: 전체 시스템 / 가용성 설계 | T, A | ST-PR-NF-RL-010 | VT-CL-001 | HAZ-SW | Open |
| MR-002 | Tier 2 | PR-NF-RL-011 | SWR-NF-RL-011 | SAD: 전체 시스템 / MTBF | A | ST-PR-NF-RL-011 | — | HAZ-SW | Open |
| MR-002 | Tier 2 | PR-NF-RL-012 | SWR-NF-RL-012 | SAD: DataLayer / WAL 구현 | T, A | ST-PR-NF-RL-012 | VT-CL-001 | HAZ-DATA | Open |
| MR-044 | Tier 2 | PR-NF-RL-013 | SWR-NF-RL-013 | SAD: SystemMgmt / 자동 복구 | T, D | ST-PR-NF-RL-013 | VT-CL-001 | HAZ-SW | Open |
| MR-019 | Tier 1 | PR-NF-RL-014 | SWR-NF-RL-014 | SAD: DicomComm / 오프라인 큐 | T, D | ST-PR-NF-RL-014 | VT-CL-001 | HAZ-DATA | Open |
| MR-003 | Tier 2 | PR-NF-RL-015 | SWR-NF-RL-015 | SAD: 전체 시스템 / 메모리 관리 | T, A | ST-PR-NF-RL-015 | — | HAZ-SW | Open |
| MR-043 | Tier 3 | PR-NF-UX-020 | SWR-NF-UX-020 | SAD: UI / 교육 자료 | D, A | ST-PR-NF-UX-020 | VT-UX-001 | — | Open |
| MR-003 | Tier 2 | PR-NF-UX-021 | SWR-NF-UX-021 | SAD: UI / 워크플로우 최적화 | T, D | ST-PR-NF-UX-021 | VT-UX-001 | HAZ-RAD | Open |
| MR-003 | Tier 2 | PR-NF-UX-022 | SWR-NF-UX-022 | SAD: UI / 터치 타겟 규격 | I | ST-PR-NF-UX-022 | VT-UX-001 | — | Open |
| MR-045 | Tier 2 | PR-NF-UX-023 | SWR-NF-UX-023 | SAD: UI / 다국어 지원 | T, I | ST-PR-NF-UX-023 | VT-UX-001 | — | Open |
| MR-042 | Tier 3 | PR-NF-UX-024 | SWR-NF-UX-024 | SAD: UI / UX 설계 | D, A | ST-PR-NF-UX-024 | VT-UX-001 | — | Open |
| MR-048 | Tier 2 | PR-NF-UX-025 | SWR-NF-UX-025 | SAD: UI / 오류 처리 흐름 | T, D | ST-PR-NF-UX-025 | VT-UX-001 | HAZ-RAD | Open |
| MR-003 | Tier 2 | PR-NF-UX-026 | SWR-NF-UX-026 | SAD: UI / Emergency 버튼 | T, D | ST-PR-NF-UX-026 | VT-UX-002 | HAZ-RAD | Open |
| MR-021 | Tier 2 | PR-NF-CP-030 | SWR-NF-CP-030 | SAD: Platform / OS 호환성 | T, I | ST-PR-NF-CP-030 | — | HAZ-SW | Open |
| MR-023 | Tier 2 | PR-NF-CP-031 | SWR-NF-CP-031 | SAD: HW_HAL / Detector 플러그인 | T, D | ST-PR-NF-CP-031 | VT-CL-001 | HAZ-RAD | Open |
| MR-023 | Tier 2 | PR-NF-CP-032 | SWR-NF-CP-032 | SAD: HW_HAL / Generator 플러그인 | T, D | ST-PR-NF-CP-032 | VT-CL-001 | HAZ-RAD | Open |
| MR-021 | Tier 2 | PR-NF-CP-033 | SWR-NF-CP-033 | SAD: DicomComm / PACS 호환성 | T, D | ST-PR-NF-CP-033 | VT-CL-001 | HAZ-DATA | Open |
| MR-012 | Tier 2 | PR-NF-CP-034 | SWR-NF-CP-034 | SAD: UI / 해상도 대응 | T, I | ST-PR-NF-CP-034 | VT-UX-002 | — | Open |
| MR-019, MR-020 | Tier 1 | PR-NF-CP-035 | SWR-NF-CP-035 | SAD: DicomComm / DICOM 표준 | T, I | ST-PR-NF-CP-035 | VT-CL-001 | HAZ-DATA | Open |
| MR-033, MR-034 | Tier 1 | PR-NF-SC-040 | SWR-NF-SC-040 | SAD: Security / 규제 준수 체계 | I, A | ST-PR-NF-SC-040 | VT-SEC-001 | HAZ-SEC | Open |
| MR-034 | Tier 1 | PR-NF-SC-041 | SWR-NF-SC-041 | SAD: Security / TLS 설정 | T, I | ST-PR-NF-SC-041 | VT-SEC-001 | HAZ-SEC | Open |
| MR-033 | Tier 1 | PR-NF-SC-042 | SWR-NF-SC-042 | SAD: Security / 비밀번호 정책 | T, I | ST-PR-NF-SC-042 | VT-SEC-001 | HAZ-SEC | Open |
| MR-033 | Tier 1 | PR-NF-SC-043 | SWR-NF-SC-043 | SAD: Security / RBAC 모델 | T, I | ST-PR-NF-SC-043 | VT-SEC-001 | HAZ-SEC | Open |
| MR-036, MR-037 | Tier 1 | PR-NF-SC-044 | SWR-NF-SC-044 | SAD: Security / SAST/DAST 통합 | T, A | ST-PR-NF-SC-044 | VT-SEC-001 | HAZ-SEC | Open |
| MR-035 | Tier 1 | PR-NF-SC-045 | SWR-NF-SC-045 | SAD: Security / Audit Log 저장소 | T, I | ST-PR-NF-SC-045 | VT-SEC-001 | HAZ-SEC | Open |
| MR-050 | Tier 1 | PR-NF-MT-050 | SWR-NF-MT-050 | SAD: 전체 아키텍처 / 모듈 분리 | I, A | ST-PR-NF-MT-050 | — | HAZ-SW | Open |
| MR-052 | Tier 1 | PR-NF-MT-051 | SWR-NF-MT-051 | SAD: 전체 / 커버리지 목표 | T, A | ST-PR-NF-MT-051 | — | HAZ-SW | Open |
...
| MR-052 | Tier 1 | PR-NF-RG-063 | SWR-NF-RG-063 | SAD: 전체 / Design Controls | I, A | ST-PR-NF-RG-063 | — | HAZ-SW | Open |
| MR-054 | Tier 1 | PR-NF-RG-064 | SWR-NF-RG-064 | SAD: DicomComm / Conformance | I | ST-PR-NF-RG-064 | — | HAZ-DATA | Open |
| MR-053 | Tier 1 | PR-NF-RG-065 | SWR-NF-RG-065 | SAD: 전체 / RTM 완전성 | I, A | ST-PR-NF-RG-065 | — | — | Open |
...
| STRIDE | Spoofing, Tampering, Repudiation, Information Disclosure, Denial of Service, Elevation of Privilege | 위협 모델링 방법론 |
...
| 기능 영역 | Carestream ImageView | Siemens syngo FLC | Canon CXDI NE | Fujifilm FDX Console | OR Technology dicomPACS | 본 제품 목표 (Phase 1) |
|-----------|---------------------|--------------------|---------------|---------------------|------------------------|----------------|
| AI 통합 | Smart Noise Cancellation | myExam IQ | — | — | AIAA, ANF | Phase 2 AI (PR-AI-xxx) |
| Image Stitching | — | MAXalign | 최대 4장 | — | MFLA | 최대 4장 (PR-IP-025, Phase 2) |
| Organ Programs | — | 1,000+ (200+ 소아) | 다수 | — | 400+ projections | 50개+ (PR-WF-010, Phase 1) |
| 터치스크린 | 지원 | 지원 | "Pinch to zoom" | 지원 | 지원 | 완전 최적화 (PR-NF-UX-022) |
| Scatter Correction | — | — | Advanced SW | — | ADPC | SW 기반 (PR-IP-033, Phase 2) |
| RDSR 지원 | 지원 | 지원 | 지원 | 지원 | OR dose inspector | 지원 (PR-DM-042, Phase 2) |
| DICOM Print | 지원 | 지원 | 지원 | 지원 | 지원 | 지원 (PR-DC-054, Phase 1) |
| CD/DVD Burning | 지원 | 지원 | 지원 | 지원 | 지원 | 지원 (PR-WF-019, Phase 1) |
...
*본 문서는 PRD-XRAY-GUI-001 v2.0이며, MRD v3.0 4-Tier 체계를 반영한 Design Input 문서입니다.*

*3단계 추적성 계층: **MR-xxx(MRD) → PR-xxx(PRD, 시스템 Design Input) → SWR-xxx(FRS/SRS, IEC 62304 §5.2 소프트웨어 요구사항)** 구조를 통해 규제 기관(FDA/CE/MFDS)의 요구사항 추적성을 완전히 충족합니다.*
