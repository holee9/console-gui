...
| 항목 | 원본 값 |
|------|---------|
| 창 스타일 | WS (WindowStyle)=None, AT (AllowsTransparency)=True |
...
### Slide 2/3 원시 토큰 → WPF (Windows Presentation Foundation) 토큰 매핑
...
| MVVM (Model-View-ViewModel) 바인딩 | PPT에 없음 (코드비하인드) | Command, Binding 완비 | PPT 초과 (긍정적) |
