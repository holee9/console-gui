...
팀이 자체 DISPATCH Status를 업데이트한다. CC는 읽기만 한다.
...
| `NOT_STARTED → TIMEOUT` | CC가 60분 미응답 후 선언 | CC만 업데이트 |
...
### CC Status 변경 금지 [S09-R3 교훈]
- [HARD] CC는 팀 DISPATCH Status(IN_PROGRESS/BLOCKED/COMPLETED)를 **임의 수정 금지**
- CC가 관리하는 것은 `_CURRENT.md`의 팀 행(IDLE/ACTIVE/MERGED)과 DISPATCH 파일의 active/↔completed/ 이동만

---

## 3. DISPATCH File Management [HARD]

DISPATCH 파일 이동·삭제는 **CC 단독 권한**.

- [HARD] 팀은 `active/`, `completed/`, `_CURRENT.md` 파일을 생성·이동·삭제 금지
- [HARD] 팀은 자체 DISPATCH 파일 **내부**의 Status 섹션만 수정
- [HARD] active/↔completed/ 이동은 CC가 머지 완료 후 수행
...
Version: 2.0.0 (team-common.md §1, §9~§11, §16에서 분리)
Effective: 2026-04-22
Cross-ref: `cc-operating-protocol.md`, `session-lifecycle.md`, `quality-standards.md`, `role-matrix.md`
