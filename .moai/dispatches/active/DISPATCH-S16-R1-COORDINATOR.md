# DISPATCH S16-R1 — Coordinator (Integration)
...
### T1: IDLE CONFIRM
- **설명**: S15-R3 MERGED 완료. 이번 라운드에서 수행할 새로운 통합 작업이 있는지 확인
- **체크리스트**:
  - [ ] _CURRENT.md에서 S16-R1 ACTIVE 확인
  - [ ] ScheduleWakeup(300초) 즉시 설정 — /clear 후 첫 액션으로
  - [ ] UI.Contracts, UI.ViewModels, App 모듈 빌드 정상 확인
...
|---------|------|------|--------|----------|-----------|------|
| T1 | IDLE CONFIRM | NOT_STARTED | Coordinator | P2 | - | - |
| T2 | ScheduleWakeup 필수 설정 | NOT_STARTED | Coordinator | P1 | - | CRITICAL |

---

## Constraints
- 소유 모듈만 수정: UI.Contracts, UI.ViewModels, App, tests.integration/
- DesignTime/ 수정 금지 (Design 단독 소유)
