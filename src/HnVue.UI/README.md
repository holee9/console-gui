...
| `LogoutCommand` | `IRelayCommand` | 세션 종료 및 로그인 화면 복귀. `ISecurityService.LogoutAsync()` fire-and-forget 호출 (2026-04-06 추가) |
| `NavigateCommand` | `IRelayCommand<string>` | 네비게이션 항목 이동 |

### WorkflowViewModel 상세 (2026-04-06 확장)

| 프로퍼티 | 타입 | 설명 |
|---|---|---|
| `CurrentWorkflowState` | `WorkflowState` | 현재 워크플로우 상태 (Idle, Exposing, Complete 등) |
| `CurrentSafeState` | `SafeState` | 현재 안전 상태 (Idle, Warning, Blocked, Emergency) |
| `SafeStateLabel` | `string` | 안전 상태 레이블 (자동 업데이트) |

`UpdateSafeStateDisplay()` — SafeState 변경 시 자동으로 UI 색상 인디케이터 업데이트 (SafeStateToColorConverter 사용)
...
| `SafeStateToColorConverter` | `HnVue.UI.Converters.SafeStateToColorConverter` | SafeState enum → SolidColorBrush 변환. Idle=녹색(#00B050), Warning=노랑(#FFC000), Blocked=주황(#FF6600), Emergency=빨강(#FF0000) |
...
| `SafeStateToColorConverterTests.cs` | 4 | SafeState → Color 변환 테스트 (Idle/Warning/Blocked/Emergency) |

**전체 테스트: 93개** (+7 SafeStateToColorConverter 및 WorkflowViewModel SafeState 확장)
