...
| v2.5 | 2026-04-20 | Minor 개정: S14-R2 Trait 수정 반영 (Team A 87개 Trait 누락 수정, 373/373 pass, Security 90.04%), QA CONDITIONAL PASS (4107/4124, 99.59%, Safety-Critical 100%) | RA팀 |
...
| CI-006 | 검출기 드라이버 SDK (5종) | 바이너리 | 아티팩트 저장소 | AbyzSdk 0.1.0.0 (자사 CsI FPD), AbyzSdk.Imaging 0.1.0.0 (자사 FPD 이미징), HME libxd2 2.0 (2G 무선 FPD, 146 exports), HME libxd 1.0 (1G FPD, 100 exports), HME CIB_Mgr 1.0 (CR/DR 노출 제어, 9 exports) |
...
| S04 | 2026-04-11~13 | CMP 승인, SBOM/RTM 갱신, PHI 암호화 | sprint/S04 |
| S05 | 2026-04-11~13 | 교차검증 S03 R3, 6팀 DISPATCH 발행 | sprint/S05 |
| S06 | 2026-04-11~13 | Detector SDK 5종 추가, SBOM v3.0/SOUP v2.1 업데이트 | sprint/S06 |
...
| S14 | 2026-04-20~ | Trait 수정 (87개 누락 수정, 373/373 pass), QA CONDITIONAL PASS (4107/4124, 99.59%), Safety-Critical 100% | sprint/S14 |
...
#### 4.7.2 현재 문서 상태 (v2.5 기준 — 2026-04-20)

| 문서 번호 | 문서명 | 현재 버전 | 상태 |
|----------|--------|----------|------|
| DOC-042 | 형상관리 계획서 (CMP) | v2.5 | **Approved** |
| DOC-019 | SBOM | v2.0 | Approved |
| DOC-033 | SOUP Report | v2.1 | Approved |
| DOC-032 | RTM | v2.8 | Draft (S14-R2 Trait 수정 반영 필요) |
...
### 4.8 백업 및 복구 절차 (Backup & Recovery)

#### 4.8.1 백업 대상 및 주기

| 백업 대상 | 백업 방식 | 주기 | 보관 기간 | 담당 |
|----------|----------|------|----------|------|
| 소스코드 저장소 (Gitea/GitHub) | Git 원격 미러링 (Gitea <-> GitHub) | 실시간 (Push 시 자동) | 무기한 (Git 이력) | SW 개발 책임자 |
| 규제 문서 (docs/) | Git 커밋 이력 + GitHub Releases | 실시간 (커밋 시) | 무기한 (Git 이력) | RA 담당자 |
| SBOM/SOUP 데이터 | Git 버전 관리 + 릴리즈 아카이브 | 변경 시 | 무기한 | RA 담당자 |
| CI/CD 아티팩트 (빌드 산출물) | GitHub Actions Artifacts + Releases | 빌드 시 | 90일 (Artifacts), 무기한 (Releases) | QA 책임자 |
| Sprint 베이스라인 태그 | Git 태그 (sprint/S{N:02d}-*) | Sprint 완료 시 | 무기한 | SW 개발 책임자 |
| 설치 패키지 (MSI) | GitHub Releases + SHA-256 해시 | 릴리즈 시 | 무기한 | SW 개발 책임자 |
| 코드서명 인증서 | HSM/암호화 볼트 백업 | 갱신 시 | 인증서 유효기간 + 1년 | PM |

#### 4.8.2 재해 복구 시나리오

| 시나리오 | 복구 방법 | 목표 복구 시간 (RTO) | 목표 복구 지점 (RPO) |
|---------|---------|-------------------|-------------------|
| 로컬 개발 환경 장애 | Git clone + NuGet 복원 + 빌드 | 2시간 | 최신 커밋 (0 손실) |
| Gitea 서버 장애 | GitHub 미러에서 복구 | 4시간 | 최신 Push (실시간 미러링) |
| GitHub 장애 | Gitea 로컬 저장소에서 복구 | 4시간 | 최신 Push |
| 문서 손실 | Git 이력에서 복원 | 1시간 | 최신 커밋 |
| 릴리즈 산출물 손실 | GitHub Releases 아카이브에서 복구 | 2시간 | 해당 릴리즈 버전 |
| 전체 저장소 손실 | Gitea/GitHub 양쪽 교차 복구 | 8시간 | 최신 동기화 시점 |

#### 4.8.3 백업 검증

- 분기 1회 백업 복구 테스트 수행 (QA 책임자)
- 복구 테스트 결과를 형상 감사 보고서에 기록
- Sprint 베이스라인 태그 무결성 검증 (Git 태그 존재 여부 확인)
...
v2.0 대비 v2.1 개선:
1. **IEC 62304 Section 8.2.2 백업/복구 절차 신설**: Section 4.8 (백업 대상/주기, 재해 복구 시나리오, 백업 검증)
2. **CI-006 Detector SDK 5종 상세화**: AbyzSdk 2종 + HME 3종 버전 및 Export 수 명시
3. **Sprint 이력 S05~S07 추가**: Detector SDK 통합, CMP/RTM 갱신 이력 반영

### v2.5 변경 사항

v2.4 대비 v2.5 개선:
1. **S14-R2 Trait 수정 반영**: Team A 87개 Trait 누락 수정, 373/373 pass, Security 90.04%
2. **QA CONDITIONAL PASS 반영**: 4107/4124 (99.59%), Safety-Critical 100%
3. **Sprint S14 이력 추가**: Trait 수정 보완, 커버리지 개선
