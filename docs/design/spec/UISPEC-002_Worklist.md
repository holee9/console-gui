...
| ViewModel | `src/HnVue.UI/ViewModels/PatientListViewModel.cs` (Model-View-ViewModel) |
| 관련 SPEC | SPEC-UI-002 |
...
| 7 | MS (ModalitySelect) | ComboBox | min-width:80px |
| 8 | FilterDivider | — | — |
| 9 | FilterLabel "상태" | Label | — |
| 10 | SS (StatusSelect) | ComboBox | min-width:80px |
| 11 | FilterDivider | — | — |
...
| 새로등록 버튼 | Primary 버튼 | `HPB (HnVue.PrimaryButton)` 스타일 | 구현됨 |
| 현재촬영 버튼 | Secondary 버튼 | `HOB (HnVue.OutlineButton)` (IsEnabled=False) | 버튼 존재, 기능 없음 |
| 판독완료 버튼 | Secondary 버튼 | `HOB (HnVue.OutlineButton)` (IsEnabled=False) | 버튼 존재, 기능 없음 |
