...
| Phase | 예상 공수 (SW 2명 기준) | 핵심 MR | 목표 |
|-------|------------------------|---------|------|
| **Phase 1** | ~24-36 man-month | P1 전체 + P2 전체 + 핵심 P3 | feel-DRCS 핵심 기능 대체, MFDS 인허가 |
| **Phase 2** | ~18-24 man-month (인력 보강 전제) | 잔여 P3 + 일부 P4 | 업계 표준 달성, FDA/CE 준비 |
| **Phase 3** | TBD | 잔여 P4 | AI/Cloud 고급 기능 |

### 8.2 Phase 1 기능 범위

Phase 1에서 구현하는 MR (P1 + P2 + 핵심 P3):

- **DICOM**: C-STORE, MWL, MPPS, Storage Commitment, Q/R (MR-019, 020)
- **워크플로우**: MWL 자동 조회, 수동 등록, STAT 처리, 프로토콜 50개 (MR-001~005, 007, 009)
- **영상**: 자동 영상처리(외부 SDK), W/L/Zoom/Pan, 후처리 Preset, Grid Line 제거 (MR-011~013)
- **디텍터/Generator**: 자사 디텍터 통합, Generator 연동 (MR-010, 031)
- **선량**: RDSR, DRL 경고, AEC 모니터링 (MR-027~032)
- **사이버보안**: RBAC, PHI 암호화, 감사 로그, SBOM, CVD, 코드 서명 (MR-033~040)
- **규제**: IEC 62304/62366, MFDS (MR-050~055)
- **자동화**: Auto Labeling, Auto Crop (MR-070, 071)

### 8.3 Phase 2 추가 기능

- Auto Stitching 최소 3장 (MR-015) — 경쟁사 표준
- Reject Analysis (MR-016)
- 멀티벤더 디텍터 5개+ (MR-023)
- 다국어 8개+ (MR-045)
- EMR Bridge (MR-068)
- FDA 510(k) / CE 마킹
